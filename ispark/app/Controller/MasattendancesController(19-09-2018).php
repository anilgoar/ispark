...
class MasattendancesController extends AppController 
{
    ...
    public function uploadattend(){
        $this->layout = "home";
       // $AttandDate="2018-02-28";
        //$this->sendattendance($AttandDate);die;
        $wrongData = array();
        
       $branch= $this->Masjclrentry->find('list',array('fields'=>array('BranchName')));
       //print_r($branch);die;
       //$this->set('AttandDate', $this->Masattandance->find('all',array('fields'=>array('max(AttandDate)'))));
...
date_add($date1,date_interval_create_from_date_string("1 day"));
 ...
 if($row[5]=='A'||$row[5]=='P'||$row[5]=='DH'||$row[5]=='HD')
 {
  ...
  $Emp = $this->Masattandance->query("SELECT EmpCode from masjclrentry where BioCode = '{$row[0]}' limit 1");
  //print_r($Emp);
  $pendings = $this->Masattandance->query("SELECT * from Attandence where PendingStatus = 1 and  BioCode= '{$row[0]}' limit 1");
    ...
    public function pendingemp(){
        ...
        if($this->request->is('POST')){
            ...
            $Bio1= $Biometric==''?"":"and a1.BioCode='$Biometric'";
            $d2 = $this->Masattandance->query("SELECT * from Attandence where date(AttandDate)=curdate()-1 and BranchName= '$Branch' and EmpCode is null and PendingStatus = 0 and  EmpStatus = 'InHouse' $Bio group by BioCode");
            $this->set('data',$d2);
            ...
            if(!empty($chk)){
                ...
                foreach ($chk as $c){
                    ...
                    $d21 = $this->Masattandance->query("SELECT * from Attandence where date(AttandDate)=curdate()-1 EmpCode is null and PendingStatus = 0 and BioCode='$c'");
                    $Data['BioCode']=$c;
            ...
            $d21 = $this->Masattandance->query("SELECT * from Attandence a1 inner join mas_Jclrentrydata mj on a1.BioCode=mj.BioCode where a1.BranchName= '$Branch' and a1.EmpCode is null and a1.PendingStatus = 1 $Bio1 group by a1.BioCode");  
            $this->set('data1',$d21);
