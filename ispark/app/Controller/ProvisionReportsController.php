...
//        }
    ...
    $data = $this->Provision->query("SELECT cm2.OPBranch,cm2.process_name,pm.month,pm.cost_center,SUM(pm.provision) provision, GROUP_CONCAT(IF(tab.billRaised IS NULL,'0',tab.billRaised)) billRaised, 
...
GROUP_CONCAT(ti.po_remarks,ti.grn_remarks) `ActionRemarks`
 FROM tbl_invoice ti INNER JOIN cost_master cm ON cm.cost_center = ti.cost_center WHERE ti.finance_year='$year' and left(ti.month,3)='$month' and ti.status='0' $branch1
 ...
 ORDER BY $reportType");
    ...
    $data1 = $this->Provision->query("select cm2.OPBranch,cm2.process_name,pp.FinanceMonth,pp.Cost_Center_OutSource,sum(outsource_amt)outsource_amt,sum(if(Processed='1',outsource_amt,0)) processed from `provision_particulars` pp inner join `provision_master` pm
...
GROUP_CONCAT(ti.po_remarks,ti.grn_remarks) `ActionRemarks`
 FROM tbl_invoice ti INNER JOIN cost_master cm ON cm.cost_center = ti.cost_center WHERE ti.finance_year='$year' and left(ti.month,3)='$month' and ti.status='0' $branch1
  GROUP BY ti.`month`, ti.cost_center)AS tab ON pm.cost_center = tab.cost_center AND pm.month = tab.month
  INNER JOIN cost_master  cm2 ON pm.cost_center =  cm2.cost_center $branch2
  WHERE pm.finance_year='$year'  and left(pm.month,3)='$month' 
   GROUP BY pm.branch_name,pm.month,pm.cost_center
 ORDER BY $reportType");
