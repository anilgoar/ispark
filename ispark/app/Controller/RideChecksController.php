...
class RideChecksController extends AppController {
    ...
    public function index() {
        ...
        $rides = $this->PageMaster->query("SELECT * FROM pages_ride WHERE user_name='" . $id . "'");
    ...
    public function save() {
        ...
        $ch = explode(",", $ride);
        $q1 = "SELECT id,parent_id from pages_master WHERE ";
        foreach ($ch as $ot) {
            $q1.="id='$ot' OR ";
        }
        $q1 = substr($q1, 0, -4);
        ...
        $child = "";
        foreach ($dd as $row) {
            if ($row['pages_master']['parent_id'] > 0) {
                //$p.=$row['pages_master']['parent_id'].",";
                array_push($p, $row['pages_master']['parent_id']);
                $child.=$row['pages_master']['id'] . ",";
                //array_push($ch,$row['pages_master']['id']);
            } else {
                //$p.=$row['pages_master']['id'].",";
                array_push($p, $row['pages_master']['id']);
            }
        }
        $pp = implode(",", array_unique($p));

        /* $check1 = "SELECT parent_id from pages_master WHERE ";
          foreach($ch as $row){
          $check1.="id='$row' OR ";
          }
          ...
          if(in_array($obj_e['pages_master']['parent_id'], $ch)){
          $arr_check = true;
          }else{
          $arr_check = false;
          }
        ...
        $check = $this->PageMaster->query("select id from pages_ride WHERE user_name='" . $username . "'");
        if ($check[0]['pages_ride']['id'] == "") {
            $rides = $this->PageMaster->query("INSERT INTO pages_ride set user_name='" . $username . "', access='" . $child . "',parent_access='" . $pp . "'");
        } else {
            $rides = $this->PageMaster->query("Update pages_ride set access='" . $child . "',parent_access='" . $pp . "' WHERE user_name='" . $username . "'");
        }


        /* $email_id = $this->Session->read('email');
          $obj = $this->PageMaster->query("SELECT access,parent_access FROM pages_ride WHERE user_name='$email_id'");
