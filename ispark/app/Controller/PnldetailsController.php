...
class PnldetailsController extends AppController 
{
    ...
    public function save_record()
    {
        if($this->request->is('POST'))
        {
            ...
            $seat = addslashes($this->request->data['seat']);
            ...
            if($EntryType=='Process')
            {
               ...
               else
               {
                   ...
                   $NewData['PnlProcessSave']['Seat'] = $seat;
                   $NewData['PnlProcessSave']['Rate'] = $rate;
               ...
               echo '<table class="table">';
                       ...
                       foreach($ProcessDetails as $processDet)
                       {
                           ...
                           echo '<td>'.$processDet['PnlProcessSave']['Seat'].'</td>';
                           echo '<td>'.$processDet['PnlProcessSave']['Rate'].'</td>';
    ...
    public function get_pnl_branch()
    {
        ...
        echo '<table class="table">';
                ...
                $BranchDetails = $this->PnlBranchSave->find('all',array('conditions'=>"pnlMasterId='$DescId' and FinanceYear='$FinanceYear' and FinanceMonth='$FinanceMonth' $branch"));
                foreach($BranchDetails as $branchdet)
                {
                    echo '<tr>';
                    echo '<td>'.$branchdet['PnlBranchSave']['Branch'].'</td>';
    ...
    public function pnl_records_add_branch()
    {
        $this->layout='home';
        $BranchMaster = $this->Addbranch->find('list',array('fields'=>array('id','branch_name'),'conditions'=>"active=1"));
        $PnlMaster = $this->PnlMaster->find('all',array('conditions'=>"active=1"));
