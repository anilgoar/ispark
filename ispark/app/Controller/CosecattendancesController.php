...
{
    public $uses = array('Addbranch','User','Masattandance','Masjclrentry','CostCenterMaster','Design','Leave','MasJclrentrydata','DepartmentNameMaster','DesignationNameMaster','ProcessAttendanceMaster','AccessPages');
    ...
    public function beforeFilter()
    {
        ...
        else
        {
            ...
            $this->Auth->allow('index','discardattandence','pendingemp','uploadattend','exportsalary','get_design','incentive',
                    ...
                    'showfile','discardsalary','discardincentive','getdept','delete_empty_emply','sendattendance','discardattandence1','getcostcenter');
    ...
    public function uploadattend(){
        ...
        if($this->request->is('POST')){
            ...
            $AttandDate2 = $this->request->data['Masattendance']['AttandDate'];
            $startdate = $AttandDate=  date_format(date_create($AttandDate2),'Y-m-d'); 
            $count = 1;
            if($AttandDate==$startdate){
                if(($FileTye=='text/csv' || $FileTye=='application/csv' || $FileTye=='application/vnd.ms-excel' || $FileTye=='application/octet-stream') && strtolower(end($info)) == "csv"){
                    ...
                    if($flag)
                    {
                        ...
                        #print_r($Emp);exit;
                        $cost_master = $this->CostCenterMaster->find("first",array("conditions"=>"cost_center='".$Emp[0]['masjclrentry']['CostCenter']."'",'fields'=>array('id','cost_center'))); 
                        $rides = $this->AccessPages->query("SELECT * FROM acess_pages WHERE user_id='$user' and cost_id='" .$cost_master['CostCenterMaster']['id']. "' and att_upload='1'");
                        if(empty($rides))
                        {
                            /*print_r($user);
                            echo "<br/>";
                            print_r($Emp[0]['masjclrentry']['CostCenter']);
                            echo "<br/>";
                            print_r($cost_master['CostCenterMaster']['id']);
                            exit; */
                            return $this->Session->setFlash('Not Right to Upload Attendance for biocode - '."{$row[0]}". ' and cost center '.$cost_master['CostCenterMaster']['cost_center']);
                        } 
                        //echo "SELECT * from Attandence where date(AttandDate)='$AttandDate' and BioCode= '{$row[0]}'   limit 1";exit;
                        $attn_exist = $this->Masattandance->query("SELECT * from Attandence where date(AttandDate)='$AttandDate' and BioCode= '{$row[0]}'   limit 1");
                        if(empty($attn_exist))
                        {
                            ...
                            //print_r($checkNa);exit;
                            //    echo "SELECT EmpCode,CostCenter from masjclrentry where BioCode = '{$row[0]}' limit 1";
                            $Emp = $this->Masattandance->query("SELECT EmpCode,CostCenter from masjclrentry where BioCode = '{$row[0]}' limit 1");
                            #print_r($Emp);
                    ...
                    if($checkbarnch){
                        ...
                        if($checkstatus){
                            ...
                            //echo $Emp[0]['masjclrentry']['CostCenter'];
            #$cost_master = $this->CostCenterMaster->find("first",array("conditions"=>"cost_center='".$Emp[0]['masjclrentry']['CostCenter']."'",'fields'=>array('id','cost_center'))); 
           // print($cost_master[0]);
            #echo "SELECT * FROM acess_pages WHERE user_id='$user' and cost_id='" .$cost_master['CostCenterMaster']['cost_center']. "' and att_upload='1'";exit;
            //print_r($cost_master);exit;
            //echo "SELECT * FROM acess_pages WHERE user_id='$user' and cost_id='" .$cost_master['CostCenterMaster']['id']. "' and att_upload='1'";exit;
            #$rides = $this->AccessPages->query("SELECT * FROM acess_pages WHERE user_id='$user' and cost_id='" .$cost_master['CostCenterMaster']['id']. "' and att_upload='1'");
            //print_r($rides);exit;
            $file_upload = false;
                ...
                if(isset($this->request->data['mail']) && $this->request->data['mail']="mail"){
                    ...
                    if($file_upload)    
                    {
                        $this->Session->setFlash("Data Imported Successfully on Date $AttandDate2");
                    }
                    else
                    {
                        $this->Session->setFlash("Data Imported Failed on Date $AttandDate2");
                    }
    ...
    public function sendattendance($AttandDate){
        ...
        */
        ...
        foreach($emailArr AS $r1){
            ...
            $filename="/var/www/html/mascallnetnorth.in/ispark/app/webroot/csv_file/$fn".$autoid.rand(100000,999999).date('dmYHis').".xls";
    ...
    public function discardattandence_old(){
        $this->layout = "home";
        $d2 = $this->Masattandance->query("SELECT max(date(AttandDate)) AttandDate from Attandence");
        $nextdate1=date_create($d2[0][0]['AttandDate']);
     ...
     if($this->request->is('POST'))
        ...
        public function discardattandence(){
                ...
                {
                    $AttandDate     =   $this->request->data['upload']['AttandDate'];
                    $branchname     =   $this->request->data['upload']['branch_name'];
                    ...
                    if($this->Masattandance->deleteAll(array('AttandDate >='=>$AttandDate,'CostCenter'=>$costcenter,'BranchName'=>$branchname)))
                    {
                            $this->Session->setFlash('Attendance Discard Successfully');
                            return $this->redirect(array('action' => 'discardattandence'));  
                    }
                        ...
                        $this->Session->setFlash('Attendance Not Discard Please Try Again.');
                        return $this->redirect(array('action' => 'discardattandence1')); 
                    }
...
public function get_design()
    ...
    public function getcostcenter(){
        if(isset($_REQUEST['BranchName']) && $_REQUEST['BranchName'] !=""){
            ...
            if(!empty($data)){
                //echo "<option value=''>Select</option>";
                echo "<option value=''>Select CostCenter</option>";
                foreach ($data as $val){
                    echo "<option value='$val'>$val</option>";
                }
                die;
