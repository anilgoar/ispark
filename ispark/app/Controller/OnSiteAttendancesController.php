    ...
    public function markfield(){
        ...
        if(isset($_REQUEST['CSN'])){
            ...
            foreach($dar as $row){
                ...
                $AttandMonth    =   date('m-Y', strtotime('last month'));
                $whac           =   $this->Masjclrentry->query("SELECT * FROM WorkHomeAttandenceCalculation WHERE EmpCode='{$row['Masjclrentry']['EmpCode']}' AND AttandMonth='$AttandMonth';"); 
                
                $TotalDays      =   $whac[0]['WorkHomeAttandenceCalculation']['TotalDays'];
                if(empty($whac)){
                    $fieldArr[]=array(
