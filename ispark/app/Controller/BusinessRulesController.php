...
class BusinessRulesController extends AppController {
    public $uses = array('AccessPages','CostCenterMaster','Addbranch','BusinessTickets','User','DepartmentNameMaster','BusinessCommunity','BusinessGratitude','Masjclrentry','EmpOnService','OnboardLeaveAlert','BusinessRule','CustomerChat');
        
    public function beforeFilter(){
        parent::beforeFilter(); 
        $this->Auth->allow('index','dashboard','dashboard_new','getempname','delete_rule','rule','close_ticket','report','export_report','edit_rule','business_community','export_business_community',
    ...
    public function rule(){
        ...
        $ticket_arr = $this->BusinessRule->find("all");
        ...
        if($this->request->is('Post')){
            ...
            {  
                ...
                #print_r($this->request->data);die;
                $data = $mail['BusinessRules'];

                $branch_name = $data['branch'];
                $department = $data['department'];
                $type = $data['type'];
                $Empname    =   implode(",", $mail['Empname']);
                $costcenter = $this->request->data['costcenter'];

                $exist_mail = $this->BusinessRule->find('first',array('conditions'=>array('branch'=>$branch_name,'costcenter'=>$costcenter,'type'=>$type)));
                ...
                else
                {
                    ...
                    $data['emp_rights'] = $Empname;
                    $data['created_at'] = date('Y-m-d H:i:s');
                    #print_r($data);die;
                    $this->BusinessRule->save($data);
    ...
    public function delete_rule()
    {
        if(isset($_REQUEST['Id']) && $_REQUEST['Id'] !=""){
            ...
            $this->BusinessRule->query("DELETE FROM business_rule WHERE id='{$_REQUEST['Id']}'");   
    ...
    public function edit_rule(){
        ...
        if(!empty($ticket_arr))
        {
            ...
            foreach($ticket_arr as $d)
            {
                $costcenter = $d['BusinessRule']['cost_center'];
