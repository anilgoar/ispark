<?php
class ChangeDojsController extends AppController {
    public $uses = array('Addbranch','Masjclrentry','Masattandance','MasJclrMaster','User','StateMaster','DepartmentNameMaster','CostCenterMaster','EmployeeSourceMasters','Masdocfile');
        
    public function beforeFilter(){
        parent::beforeFilter(); 
        $this->Auth->allow('index','viewdetails','getcostcenter','editdetails','newjclr','check_date');
    ...
    public function index(){
        ...
        if($this->request->is('Post')){ 
            $branch_name    =   $this->request->data['ChangeDojs']['branch_name'];
    ...
    public function newjclr(){
        ...
        if($this->request->is('Post')){
            ...
            // print_r($dataArr);die;     
            $UpdArr=array(
                'EmpLocation'=>"'".trim(addslashes($dataArr['ChangeDojs']['EmpLocation']))."'",
                'DOJ'=>"'".date('Y-m-d',strtotime($dataArr['DOJ']))."'",
                'BioCode'=>"'".trim(addslashes($dataArr['BioCode']))."'"
            );
            //print_r($UpdArr);die;
            if ($this->Masjclrentry->updateAll($UpdArr,array('id'=>$dataArr['MasJclrsId']))){
                
                $this->Session->setFlash('<span style="color:green;font-weight:bold;">Employee Data Updated Successfully.</span>'); 
                return $this->redirect(array('controller' => 'ChangeDojs','action'=>'index'));
            }
            
        }
    }

    public function save_doj()
    {
