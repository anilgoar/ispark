...
class UsersController extends AppController {
	public $uses=array('PageMaster','User','Access','Pages','Addbranch','InitialInvoice','CostCenterMaster','Provision','Logx','HisAction','NotificationMaster','EcrMaster','Masjclrentry','IsparkDepartmentMaster','IsparkProcessMaster');
    public function beforeFilter() 
    {
        parent::beforeFilter();
        $this->Auth->allow('add','login_new','manage_access','view_access','create_User','view','assign_Access','view_access','edit_User','add_date','getMessage','getMessageDisplay','updateMessage','user_array','puser_array','change_password','get_emp','sendmail','getcostcenter');
...
//        }
    ...
    if($this->request->is('Post')){
        ...
        else
        {
            ...
            isset($data['User']['Access_Type'])&& $data['User']['Access_Type']=="Own"?$data['User']['Access_Rights'] = $data['User']['emp_code']:"";
            isset($data['User']['Access_Type'])&& $data['User']['Access_Type']=="CostCentre"?$data['User']['Access_Rights'] = implode(",",$data['Access_Rights']):"";
    ...
    public  function edit_Users()
    {
        ...
        if($this->request->is('Post'))
        {
            ...
            if($this->User->updateAll($dataX,array('id'=>$id)))
            {
                ...
                $this->Session->setFlash(__('Data Update Successfully'));
            }
            unset($data); unset($dataX);
...
public function getcostcenter(){
        if(isset($_REQUEST['BranchName']) && $_REQUEST['BranchName'] !=""){
            ...
            $Access_Rights  =   explode(",",$_REQUEST['Access_Rights']);
            if(!empty($data)){
                echo '<div class="col-sm-2">Access Rights</div>';
                echo '<div class="col-sm-4" style="overflow-y:scroll;height:200px;" >';
                foreach ($data as $val){

                    $checked =in_array($val,$Access_Rights)?"checked":"";
                    
                    echo "<input type='checkbox' $checked name='Access_Rights[]' id='Access_Rights' value='$val' >  $val <br/>";
                }
                echo '</div>';die;
            }
            else{
