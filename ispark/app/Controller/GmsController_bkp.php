<?php
	class GmsController extends AppController 
	{
		public $uses=array('Collection','Addbranch','BillMaster','TMPCollection','Addcompany','TMPCollectionParticulars','OtherTMPDeduction','OtherDeduction',
                    ...
                    'ExpenseEntryParticular','GrnBranchAccess','ExpenseEntryApproveParticular','ExpenseEntryApproveMaster','GrnPaymentProcessing','StateList');
		...
		public function beforeFilter()
		{
			...
			$this->Auth->allow('get_collection_data','index');
                                    ...
                                    $this->Auth->allow('add'); 
                                    $this->Auth->allow('approve_imprest_tmp21'); 
                                    $this->Auth->allow('index','get_bill_remark','get_branch','getCostCenter','get_budget','add_field_value',
                                            ...
                                            'edit_grn_tmp_branch','save_grn_tmp1_branch','view_grn','view_grn_tmp','view_imprest','view_imprest_tmp','get_due_date','approve_grn2','approve_grn_tmp2','approve_imprest2','approve_imprest_tmp2');
...
//                    $flag = true;
        ...
        public function approve_grn_tmp()
        {
            ...
            if($data = $this->ExpenseEntryApproveMaster->find('first',array('fields'=>array('FinanceYear','FinanceMonth','HeadId','SubHeadId','Vendor','bill_no',
                'bill_date','Amount','Description','ExpenseDate','EntryStatus','CompId','grn_file','Id','round_off','multi_month_check','multi_month','gst_enable','due_date'),'conditions'=>array('Id'=>$Id))))
            {
...
//                    }
      ...
      public function approve_grn2()
        ...
        public function approve_grn_tmp2()
        {
            ...
            $this->set('ExpenseId',$Id);
            if($data = $this->ExpenseEntryApproveMaster->find('first',array('fields'=>array('FinanceYear','FinanceMonth','HeadId','SubHeadId','Vendor','bill_no',
                'bill_date','Amount','Description','ExpenseDate','EntryStatus','CompId','grn_file','Id','round_off','multi_month_check','multi_month','gst_enable'),'conditions'=>array('Id'=>$Id))))
            {
...
//                            }
                ...
                else
                {
                    ...
                    $ExpenTmp = $this->ExpenseEntryApproveMaster->find('first',array('conditions'=>array('Id'=>$ExpenseId)));
                    $dataY= $ExpenTmp['ExpenseEntryApproveMaster'];
                    $this->ExpenseEntryApproveMaster->updateAll(array('ApprovedBy'=>null,'ApprovalDate'=>null,'RejectRemarks'=>"'".$RejectRemarks."'",'Reject'=>'0','ApprovalDate'=>null,'RejectDate'=>"'".date('Y-m-d H:i:s')."'",'RejectBy'=>"'".$this->Session->read('userid')."'"),array('Id'=>$ExpenseId));
...
INNER JOIN tbl_user tu ON eemApp.userid = tu.Id INNER JOIN company_master cm ON eemApp.CompId = cm.Id WHERE eemApp.ExpenseEntryType='Imprest' AND (Reject = '1' || Reject='2')")); 
            }
            else
            {
                $this->set('data',$this->ExpenseEntryApproveMaster->query("SELECT eemApp.Id,tu.username,cm.company_name,eemApp.Amount,eemApp.RejectRemarks,eemApp.Reject,eemApp.ApprovalDate FROM `expense_entry_master_approve` eemApp
    ...
    INNER JOIN (SELECT * FROM tbl_grn_access where UserId='$userid' GROUP BY BranchId,UserId) tga ON eep.BranchId = tga.BranchId
     WHERE eemApp.ExpenseEntryType='Imprest' AND (eemApp.Reject = '1'  || eemApp.Reject='2') and eemApp.ApprovalDate is null GROUP BY eemApp.Id")); 
            }
        }
        ...
        if($this->request->is('POST'))
            {
                ...
                else
                {
                    ...
                    $this->ExpenseEntryApproveMaster->query("INSERT INTO `expense_master_reject` SET ExpenseId='$ExpenseId',"
                            ...
                            . "BranchId='".$dataZ['ExpenseEntryApproveParticular']['BranchId']."',FinanceMonth='".$dataY['FinanceMonth']."',FinanceYear='".$dataY['FinanceYear']."',CompId='".$dataY['CompId']."',RejectBy='$userid',RejectDate=now()");
                   $this->redirect(array('action'=>'approve_imprest'));
                   $this->Session->setFlash(__("Imprest Rejected and Moved To User Bucket")); 
                }
            }
    ...
    public function edit_grn_tmp_branch()
        {
			...
			if($data = $this->ExpenseEntryApproveMaster->find('first',array('fields'=>array('FinanceYear','FinanceMonth','HeadId','SubHeadId','Vendor','bill_no',
                            'bill_date','Amount','Description','ExpenseDate','EntryStatus','CompId','grn_file','RejectRemarks','round_off','gst_enable'),'conditions'=>array('Id'=>$ExpenseId,'userid'=>$userid))))
			{
                               $VendorId = $data['ExpenseEntryApproveMaster']['Vendor'];
                                $HeadId = $data['ExpenseEntryApproveMaster']['HeadId'];
