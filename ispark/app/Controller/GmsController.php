<?php
	...
	{
		public $uses=array('Addbranch','BillMaster','TMPCollection','Addcompany','TMPCollectionParticulars','OtherTMPDeduction','OtherDeduction',
		...
		public function beforeFilter()
		{
                    ...
                    else
                    {
                        ...
                        $this->Auth->allow('index','add','get_branch','getCostCenter','get_budget','add_field_value',
                            'add_grn_tmp','delete_grn','get_head','get_sub_heading','imprest_entry','get_sub_heading1','imprest_add',
                            'back','back1','approve_grn','approve_grn_tmp','approve_imprest','approve_imprest_tmp','update_grn_tmp',
                            'save_payment_processing','save_payment_processing1','imprest_add','pending_grn','vendorstatuscode','delete_grn_request','get_grn',
...
. "eem.FinanceYear='$FinanceYear' AND eem.FinanceMonth='$FinanceMonth' AND bm.Id='$BranchId' AND eem.HeadId='$HeadId' AND eem.SubHeadId='$SubHeadId'");
                        ...
                        if($Balance)
                        {
                            echo round($Budget).",".round($Consume).",".round($Balance).",".round($VendorRelation);
                        }
                        else
                        {
                            echo "1";
    	...
    	public function index() 
		{
            ...
            'order' => array('branch_name' => 'asc')))); 
                        ...
                        WHERE tv.active=1 and UserId = '$userid' AND tv.Id>660 AND tv.vendor !='Previous Entry' and tv.active=1
                            ...
                            'bill_date','Amount','Description','ExpenseDate','EntryStatus','CompId','multi_month_check','multi_month','gst_enable','due_date','Vendor_State_Code','Billing_State_Code'),'conditions'=>array('userid'=>$userid))))
			{
                               ...
                               $HeadId = $data['TmpExpenseEntryMaster']['HeadId'];
                                ...
                                $compNew = $this->Addcompany->find('list',array('fields'=>array('id','comp_code'),
...
//                                }
        ...
        public function add()
        {
            ...
            $dataX['gst_enable'] = $data['gst_enable'];
            ...
            $branchIdCheck = $this->ExpenseEntryMaster->query("SELECT ep.branchid,SUM(ep.amount)total FROM tmp_expense_entry_particular ep INNER JOIN tmp_expense_entry_master em ON ep.userid = em.userid WHERE em.userid='$userid' GROUP BY ep.branchid");
            $TotalAmountB = $branchIdCheck['0']['0']['total']; 
            $branchIdChecksB = $branchIdCheck['0']['ep']['branchid'];
             
            $TotalB = $this->ExpenseMaster->find('first',array('fields'=>array('Amount','Id'),'conditions'=>
                array('BranchId'=>$branchIdChecksB,'FinanceYear'=>$FinanceYear,'FinanceMonth'=>$FinanceMonth,'HeadId'=>$HeadId,'SubHeadId'=>$SubHeadId)));
...
//                    $flag = true;
            ...
            {
                $this->Session->setFlash(__("Business Case Amount is Less. Please Contact To Admin"));    
                $this->redirect(array("controller"=>"Gms","action"=>"index"));
            }
        ...
        public function approve_grn_tmp()
        {
            ...
            if($data = $this->ExpenseEntryApproveMaster->find('first',array('fields'=>array('FinanceYear','FinanceMonth','HeadId','SubHeadId','Vendor','bill_no',
                'bill_date','Amount','Description','ExpenseDate','EntryStatus','CompId','grn_file','Id','round_off','multi_month_check','multi_month','gst_enable','due_date','Vendor_State_Code','Billing_State_Code'),'conditions'=>array('Id'=>$Id))))
            {
...
//                    }
            ...
            if($this->request->is('POST'))
            {
                ...
                else
                {
                    ...
                    $this->ExpenseEntryApproveMaster->updateAll(array('RejectRemarks'=>"'".$RejectRemarks."'",'Reject'=>'0',
                        'RejectDate'=>"'".date('Y-m-d H:i:s')."'",
                        'RejectBy'=>"'".$this->Session->read('userid')."'"),array('Id'=>$ExpenseId));
                   ...
                   $this->redirect(array('action'=>'approve_grn'));
                   $this->Session->setFlash(__("GRN Rejected and Moved To Process Head(Branch) Bucket")); 
                }
            }
        ...
        public function approve_grn_tmp2()
        {
            ...
            WHERE teep.ExpenseEntry='$Id'"));
                  ...
                  $VendorId = $data['ExpenseEntryApproveMaster']['Vendor']; 
                   ...
                   $SubHeadId = $data['ExpenseEntryApproveMaster']['SubHeadId'];
                    ...
                    $vend = $this->VendorMaster->find('list',array('fields'=>array('Id','vendor'),
                        'conditions'=>array('Id'=>$VendorId)));
                    $CompId = $data['ExpenseEntryApproveMaster']['CompId'];


                    $this->set('gstType',$this->ExpenseEntryApproveMaster->query("SELECT IF(vm.as_bill_to=1,'state',
                        IF(bm.branch_state=vm.state,'state','central'))GSTType,tscgd.GSTEnable FROM `expense_entry_master_approve` 
                    em INNER JOIN expense_entry_particular_approve eep ON em.Id=eep.ExpenseEntry  
                    INNER JOIN cost_master cm ON cm.Id = eep.CostCenterId
                    INNER JOIN branch_master bm ON cm.branch = bm.branch_name
                    INNER JOIN tbl_vendormaster vm ON vm.Id = em.vendor
            ...
            if($this->request->is('POST'))
            {
                ...
                if(!empty($this->request->data['Approve']))
                {
                    ...
                    {
                        $NewList[$un['UNList']['HeadId']] =$un['UNList']['StartWith'];
                    }

                    if(!empty($NewList))
                    {
                        $UniqueHead = $NewList[$HeadId];
                        $autoArr = $this->ExpenseEntryMaster->query("Select max(uid_auto)uid_auto from expense_entry_master where HeadId='$HeadId' and UniqueHead='$UniqueHead'"); 
                    }
                    else
                    {
                        $UniqueHead = 'Vend';
    ...
    //                            }
        ...
        public function view_approve_grn_finance_head()
        {
            $this->layout="home";
            $userid = $this->Session->read('userid');
            ...
            $this->set('data',$this->ExpenseEntryApproveMaster->query("SELECT eemApp.Id,tu.username,cm.company_name,vm.vendor,
                eemApp.Amount,eemApp.Reject,eemApp.RejectRemarks,tuR.username  RejectBy
            ...
            Where eemApp.ExpenseEntryType='Vendor' and (eemApp.reject='0' || eemApp.reject='2' || eemApp.reject='3' || eemApp.reject='4')")); 
        }
        ...
        public function save_approve_grn_finance_head()
        {
                ...
                'bill_date','Amount','Description','ExpenseDate','EntryStatus','CompId','grn_file','Id','round_off','multi_month_check','multi_month','gst_enable','Vendor_State_Code','Billing_State_Code'),'conditions'=>array('Id'=>$Id))))
            {
                ...
                $this->set('result',$this->ExpenseEntryApproveParticular->query("SELECT teep.*,cm.Branch,cm.cost_center FROM `expense_entry_particular_approve` teep 
                    INNER JOIN cost_master cm  ON teep.CostCenterId= cm.id
                    WHERE teep.ExpenseEntry='$Id'"));
                   ...
                   $SubHeadId = $data['ExpenseEntryApproveMaster']['SubHeadId'];
                    ...
                    $this->set('gstType',$this->ExpenseEntryApproveMaster->query("SELECT IF(vm.as_bill_to=1,'state',
                        IF(bm.branch_state=vm.state,'state','central'))GSTType,tscgd.GSTEnable 
                        FROM `expense_entry_master_approve` 
                    em INNER JOIN expense_entry_particular_approve eep ON em.Id=eep.ExpenseEntry  
                    INNER JOIN cost_master cm ON cm.Id = eep.CostCenterId
                    INNER JOIN branch_master bm ON cm.branch = bm.branch_name
                    INNER JOIN tbl_vendormaster vm ON vm.Id = em.vendor
    ...
    //                            }
                            ...
                            if($this->ExpenseEntryMaster->updateAll(array('GrnNo'=> "'". addslashes($GrnNO)."'",'Remarks'=>'Approved By Finance Head','ApprovalDate'=>"'".date('Y-m-d H:i:s')."'",'ApprovedBy'=>"'".$this->Session->read('userid')."'"),array('Id'=>$DataIds)))
                            {
                                $Transaction->commit();    
                                $this->Session->setFlash(__("GRN No. ".$GrnNO." With Unique $autoVal Approved Successfully"));
                            }
                    ...
                    $this->ExpenseEntryApproveMaster->deleteAll(array('Id'=>$ExpenseId));
                    $this->ExpenseEntryApproveParticular->deleteAll(array('ExpenseEntry'=>$Id));
                    $this->redirect(array('action'=>'view_approve_grn_finance_head'));
                ...
                else
                {
                    ...
                    $dataY= $ExpenTmp['ExpenseEntryApproveMaster'];
                    $this->ExpenseEntryApproveMaster->updateAll(array('ApprovedBy'=>null,'ApprovalDate'=>null,
                        'RejectRemarks'=>"'".$RejectRemarks."'",'Reject'=>'3','ApprovalDate'=>null,
                        'RejectDate'=>"'".date('Y-m-d H:i:s')."'",'RejectBy'=>"'".$this->Session->read('userid')."'"),array('Id'=>$ExpenseId));
            ...
            if($this->request->is('POST'))
                {
                    ...
                    if(!empty($this->request->data['Approve']))
                    {
                        ...
                        if($this->ExpenseEntryApproveMaster->updateAll(array('ApprovalDate'=>"'$ApprovalDate'",'Reject'=>'1','RejectRemarks'=>"''",'ApprovedBy'=>$userid),array('Id'=>$ExpenseId)))
                        {
                            $this->Session->setFlash(__("Imprest has been approved. and moved for second approval"));
                            $this->redirect(array('action'=>'approve_imprest'));
                        }
                        else
                        {
                            $this->Session->setFlash(__("Imprest Not Approved Successfully! Please Try Again"));
                        }
                    }
                    else
                    {
                        ...
                        $this->ExpenseEntryApproveMaster->query("INSERT INTO `expense_master_reject` SET ExpenseId='$ExpenseId',"
                                ...
                                . "BranchId='".$dataZ['ExpenseEntryApproveParticular']['BranchId']."',FinanceMonth='".$dataY['FinanceMonth']."',FinanceYear='".$dataY['FinanceYear']."',CompId='".$dataY['CompId']."',RejectBy='$userid',RejectDate=now()");
                       $this->redirect(array('action'=>'approve_imprest'));
                       $this->Session->setFlash(__("Imprest Rejected and Moved To Process Head(Branch) Bucket")); 
                    }
                }

        }
        
        public function approve_imprest2()
         {
             $this->layout="home";
             $this->set('data',$this->ExpenseEntryApproveMaster->query("SELECT eemApp.Id,tu.username,cm.company_name,eemApp.Amount FROM `expense_entry_master_approve` eemApp
 INNER JOIN tbl_user tu ON eemApp.userid = tu.Id
 INNER JOIN company_master cm ON eemApp.CompId = cm.Id Where eemApp.ExpenseEntryType='Imprest' and Reject = 1 and ApprovedBy is not null")); 
         } 
...
//                                }
        ...
        public function approve_imprest_tmp21()
        {
            $this->layout="home";
        $userid = $this->Session->read("userid");
        $this->set('company_master', $this->Addcompany->find('list',array('fields'=>array('id','comp_code'),
        ...
        if($this->request->is('POST'))
                        ...
                        $UniqueHead = 'Imp';
                        $autoArr = $this->ExpenseEntryMaster->query("Select max(uid_auto)uid_auto from expense_entry_master where UniqueHead='Imp'"); 
...
//                                    }
                            ...
                            if($flag)
                            {
                                $this->Session->setFlash(__("GRN No. ".$GrnNO." with Unique No $autoVal Approved Successfully"));
                            }
                            else
                            {
                                $this->Session->setFlash(__("GRN No. Not Approved Successfully! Please Try Again"));
                            ...
                            $this->ExpenseEntryApproveMaster->deleteAll(array('Id'=>$ExpenseId));
                            $this->ExpenseEntryApproveParticular->deleteAll(array('ExpenseEntry'=>$Id));
                            $this->redirect(array('action'=>'approve_imprest2'));
                            ...
                            else
                            {
                                ...
                                $ExpenTmp = $this->ExpenseEntryApproveMaster->find('first',array('conditions'=>array('Id'=>$ExpenseId)));
                                $dataY= $ExpenTmp['ExpenseEntryApproveMaster'];
                               $this->ExpenseEntryApproveMaster->updateAll(array('RejectRemarks'=>"'".$RejectRemarks."'",'ApprovalDate'=>null,'Reject'=>'2','RejectDate'=>"'".date('Y-m-d H:i:s')."'",'RejectBy'=>"'".$this->Session->read('userid')."'"),array('Id'=>$ExpenseId));
                                ...
                                $this->ExpenseEntryApproveMaster->query("INSERT INTO `expense_master_reject` SET ExpenseId='$ExpenseId',"
                                        ...
                                        . "BranchId='".$dataZ['ExpenseEntryApproveParticular']['BranchId']."',FinanceMonth='".$dataY['FinanceMonth']."',FinanceYear='".$dataY['FinanceYear']."',CompId='".$dataY['CompId']."',RejectBy='$userid',RejectDate=now()");
                               $this->redirect(array('action'=>'approve_imprest2'));
                               $this->Session->setFlash(__("Imprest Rejected and Moved Process Head(Branch) Bucket")); 
                            }
                        }
    }
...
//                                }
        ...
        public function approve_imprest_approval_finance_head()
        {
            $this->layout="home";
        $userid = $this->Session->read("userid");
        $this->set('company_master', $this->Addcompany->find('list',array('fields'=>array('id','comp_code'),
...
//                                    }
                            ...
                            $this->ExpenseEntryApproveMaster->deleteAll(array('Id'=>$ExpenseId));
                            $this->ExpenseEntryApproveParticular->deleteAll(array('ExpenseEntry'=>$Id));
                            $this->redirect(array('action'=>'view_imprest_approval_finance_head'));
                            ...
                            else
                            {
                                ...
                                $ExpenTmp = $this->ExpenseEntryApproveMaster->find('first',array('conditions'=>array('Id'=>$ExpenseId)));
                                $dataY= $ExpenTmp['ExpenseEntryApproveMaster'];
                               $this->ExpenseEntryApproveMaster->updateAll(array('RejectRemarks'=>"'".$RejectRemarks."'",'ApprovalDate'=>null,'Reject'=>'3','RejectDate'=>"'".date('Y-m-d H:i:s')."'",'RejectBy'=>"'".$this->Session->read('userid')."'"),array('Id'=>$ExpenseId));
...
//                                }
    ...
    public function save_imprest_process_head()
    {
        if($this->request->is('POST'))
        {   
            $Id = $this->request->data['ExpenseId'];
...
Where eemApp.Reject!='0' and eemApp.ExpenseEntryType='Vendor' and eemApp.userid = '$userid'")); 
    ...
    public function view_reject_grn_process_head()
        {
            $this->layout="home";
            $userid = $this->Session->read('userid');
            $username = $this->Session->read('email');
            ...
            $this->set('data',$this->ExpenseEntryApproveMaster->query("SELECT eemApp.Id,tu.username,cm.company_name,eemApp.Amount,
                eemApp.RejectRemarks,eemApp.Reject,vm.vendor,tuR.username
                FROM `expense_entry_master_approve` eemApp
INNER JOIN tbl_user tu ON eemApp.userid = tu.Id
INNER JOIN tbl_user tuR ON eemApp.RejectBy = tuR.Id
inner join branch_master bm on bm.branch_name=tu.branch_name
left join tbl_vendormaster vm on eemApp.vendor = vm.Id
INNER JOIN company_master cm ON eemApp.CompId = cm.Id
Where (eemApp.Reject='0' || eemApp.Reject='2' || eemApp.Reject='3' || eemApp.Reject='4' )   and eemApp.ExpenseEntryType='Vendor' and bm.id in ('$branch_ids') and tu.process_head = '$username'"));
            //and tu.process_head = '$username'
    ...
    public function edit_grn_process_head()
        {
    ...
    public function save_grn_process_head()
    {
        if($this->request->is('POST'))
        {   
            $Id = $this->request->data['ExpenseId'];
            if(!empty($this->request->data['Gms']['grn_file']['name']))
            {
              ...
              $this->ExpenseEntryApproveMaster->updateAll(array('Reject'=>"4",
                  'HeadId'=>$this->request->data['Gms']['HeadId'],
                  'SubHeadId'=>$this->request->data['Gms']['SubHeadId'],
                  'Bill_No'=>"'".addslashes($this->request->data['Gms']['Bill_No'])."'",
                  'Amount'=>$this->request->data['Gms']['Amount'],
                  'description'=>"'".addslashes($this->request->data['Gms']['description'])."'",
