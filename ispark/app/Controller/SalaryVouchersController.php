...
class SalaryVouchersController extends AppController {
    ...
    public function getdownloadstatus(){
        if(isset($_REQUEST['CompanyName']) && $_REQUEST['CompanyName'] !=""){
            ...
            $WhereBranch    =   $BranchName !="ALL"?"AND Branch='$BranchName'":"";
            
            $DataArr    =   $this->SalarData->query("
            SELECT DownloadVoucher
            FROM salary_data WHERE SUBSTRING(EmpCode, 1, 3)='$CompanyName' AND RIGHT(EmpCode,1)!='C' $WhereBranch  AND DATE(SalDate)='$SalDate' GROUP BY DownloadVoucher
            ");
    ...
    public function export_report(){
        ...
        if(isset($_REQUEST['CompanyName']) && $_REQUEST['CompanyName'] !=""){
            ...
            if($_REQUEST['BranchName'] =="ALL"){
                $BranchArrayList    =   $this->Addbranch->find('list',array('fields'=>array('branch_name','branch_name'),'conditions'=>array('active'=>1),'order'=>array('branch_name')));  
            }
            else{
                $BranchArrayList    =   $this->Addbranch->find('list',array('fields'=>array('branch_name','branch_name'),'conditions'=>array('branch_name'=>$_REQUEST['BranchName'],'active'=>1),'order'=>array('branch_name')));  
            }
			
			//echo '<pre>';
			//print_r($BranchArrayList);die;
            ...
            foreach($BranchArrayList as $BranchName){
