    ...
    public function newjclr(){
        ...
        if($this->request->is('Post')){
            ...
            if($this->Session->read('role')=='admin' && $this->Session->read('branch_name') =="HEAD OFFICE"){
                $UpdArr['Desgination']="'".trim(addslashes($dataArr['Desgination']))."'";
            }
		...
		$temp = explode(".", $_FILES["file"]["name"]);
                ...
                if(move_uploaded_file($FilePath, $target_file)){
                    $ExperienceDocArray=$this->Masjclrentry->find('first',array('fields'=>array('Experience_Doc'),'conditions'=>array('id'=>$dataArr['MasJclrsId'])));
                    if($ExperienceDocArray['Masjclrentry']['Experience_Doc'] !=""){
                        $OldDoc =   $ExperienceDocArray['Masjclrentry']['Experience_Doc'].",";
		...
		$temp = explode(".", $_FILES["file"]["name"]);
           ...
           if($_REQUEST['validate_status'] =="tab3" || $_REQUEST['validate_status'] =="tab4"){
               $UpdArr['AcValidationStatus']=NULL;
               $UpdArr['AcRejectionRemarks']=NULL;
           }
            
            
            if ($this->Masjclrentry->updateAll($UpdArr,array('id'=>$dataArr['MasJclrsId']))){
                
                $Jclr_qa=$this->Masjclrentry->find('first',array('conditions'=>array('id'=>$dataArr['MasJclrsId'])));
                if(strtolower($Jclr_qa['Masjclrentry']['Desgination'])=='quality auditor')
                {
                        $url = array();
                        $url['req_id'] = '3';
                        $url['mas_code'] = $Jclr_qa['Masjclrentry']['EmpCode'];
                        $url['email'] = $Jclr_qa['Masjclrentry']['OfficeEmailId'];
                        $url['active'] = $Jclr_qa['Masjclrentry']['Status'];
...
<?php
    ...
    public function deleteexpfile(){
        ...
        $ExpdocArr  =   $this->Masjclrentry->find('first',array('fields'=>array('Experience_Doc'),'conditions'=>array('id'=>$MasJclrId)));
        $Expdoc     =   explode(",",$ExpdocArr['Masjclrentry']['Experience_Doc']);
        
        if (in_array($filename, $Expdoc)){
            $arr            =   array_diff($Expdoc, array($filename));
            $Experience_Doc =   implode(",",$arr);
            $this->Masjclrentry->query("UPDATE masjclrentry SET Experience_Doc='$Experience_Doc' WHERE id='$MasJclrId'");
            unlink($path);  
        }
