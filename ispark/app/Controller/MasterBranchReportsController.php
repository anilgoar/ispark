<?php
class MasterBranchReportsController extends AppController 
{
    ...
    public function beforeFilter()
    {
        ...
        else
        {
                ...
                if(in_array('61',$roles)){$this->Auth->allow('index','get_master_process_brief_wise','get_master_event_brief_wise');$this->Auth->allow('get_master_day_wise');$this->Auth->allow('get_master_process_wise');}
    ...
    public function index() 
    {
        ...
        foreach($costArray as $br)
        {
            ...
            $branchName = $br['cost_master']['branch'];
...
WHERE  cm.cost_center = '$costcenter' AND bpp.bill_no IS NULL AND (IF(cm.po_required='Yes' AND (ti.approve_po = '' OR ti.approve_po IS NULL),TRUE,
   IF(cm.grn='Yes' AND (ti.approve_grn = '' OR ti.approve_po IS NULL),TRUE,FALSE))) group by cm.company_name,cm.branch");
            ...
            foreach($InProcessingArray as $IPA)
            {
                if(key_exists($IPA['cm']['company_name'], $data))
                {
                    ...
                    {
                        $data[$IPA['cm']['company_name']][$IPA['cm']['branch']]['InProcess'] += $IPA['0']['grnd'];
                    }
                    else
                    {
                        $data[$IPA['cm']['company_name']][$IPA['cm']['branch']]['InProcess'] = $IPA['0']['grnd'];
            ...
            foreach($costArray as $br)
            {
               $costcenter = $br['cost_master']['cost_center'];
                ...
                foreach($ToBeBilledArray as $TBBArray)
                {
                    ...
                    {
                        $data[$TBBArray['cm']['cost_center']]['Tobebilled'] += $TBBArray['pm']['balance'];
                    }
                    else
                    {
                        $data[$TBBArray['cm']['cost_center']]['Tobebilled'] = $TBBArray['pm']['balance'];
...
//                $json[$i]['Tobebilled'] = !empty($mr['Tobebilled'])?$mr['Tobebilled']:0;
//                $json[$i]['InProcess'] = !empty($mr['InProcess'])?$mr['InProcess']:0;
//                $json[$i]['PytReady'] = !empty($mr['PytReady'])?$mr['PytReady']:0;
//                $json[$i]['PayForMonth'] = !empty($mr['PayForMonth'])?$mr['PayForMonth']:0;
//                $json[$i]['PostMonth'] = !empty($mr['PostMonth'])?$mr['PostMonth']:0;
//                $json[$i]['w1'] = !empty($mr['w1'])?$mr['w1']:0;
//                $json[$i]['w2'] = !empty($mr['w2'])?$mr['w2']:0;
//                $json[$i]['w3'] = !empty($mr['w3'])?$mr['w3']:0;
//                $json[$i]['w4'] = !empty($mr['w4'])?$mr['w4']:0;
//                $json[$i]['w5'] = !empty($mr['w5'])?$mr['w5']:0;
//                $json[$i]['w6'] = !empty($mr['month2']['w1'])?$mr['month2']['w1']:0;
//                $json[$i]['w7'] = !empty($mr['month2']['w2'])?$mr['month2']['w2']:0;
//                $json[$i++]['Total'] = $Total;
    ...
    public function get_master_process_wise()  //add branch to table
    {
        ...
        if ($this->request->is('post')) 
        {
            ...
            FROM cost_master cm INNER JOIN tbl_invoice ti
            ON cm.cost_center = ti.cost_center and cm.branch = '$branch' and cm.company_name = '$company'
            ...
            $html .='</tbody>';
            $html .='<tr><th colspan="5" align="right">Total</th><th>'.round($Total/100000,2).'</th><th></th></tr></table>';
            echo $html; exit;
        }
    }
...
(IF(cm.po_required='Yes' AND (ti.approve_po = '' OR ti.approve_po IS NULL),'PO Pending',
 ...
 date_format(IF(cm.po_required='Yes' AND (ti.approve_po = '' OR ti.approve_po IS NULL),ti.po_date,
   IF(cm.grn='Yes' AND (ti.approve_grn = '' OR ti.approve_po IS NULL),ti.grn_date,
	rm.ExpDatesPayment
   )),'%d-%b-%y') `bill_action`, 
...
");
             ...
             foreach($processArray as $pra)
             {
                ...
                $html .= '<td align="center">'.$pra['0']['bill_remarks'].'</td>';
                $Total +=$pra['0']['grnd'];
             }
               
             if(empty($processArray)) { $html .= "<tr><td colspan='4'>No Events Found</td></tr>";}
             $html .='</tbody>';
             $html .='<tr><th colspan="2" align="right">Total</th><th>'.round($Total/100000,2).'</th><th colspan="4"></th></tr></table>';
            echo $html; exit;
            //$this->set("master_report",$data);
...
");
    ...
    public function edit()  //edit the window to server
    {
        if ($this->request->is('post')) 
        {
