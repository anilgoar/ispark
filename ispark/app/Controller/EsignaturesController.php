<?php
class EsignaturesController extends AppController {
    public $uses = array('Addbranch','Masjclrentry','Masattandance','MasJclrMaster','User','Masdocfile');
        
    public function beforeFilter(){
        parent::beforeFilter(); 
        $this->Auth->allow('index','viewdetails','getcostcenter','update_esignature_status','updatestatus');
    ...
    public function index(){
        ...
        if($this->request->is('Post')){ 
            $branch_name    =   $this->request->data['Esignatures']['branch_name'];
            ...
            $data   =   $this->Masjclrentry->find('all',array('fields'=>array('id','OfferNo','EmpCode','EmpName','DOJ','documentDone','Desgination','EmpLocation','EsignatureValidateStatus'),'conditions'=>$conditoin)); 
    ...
    public function viewdetails(){
        ...
        if(isset($_REQUEST['ON'])){
			$OfferNo	= 	base64_decode($_REQUEST['ON']);
			$EmpCode 	= 	base64_decode($_REQUEST['EC']);
			
            $DocArr 	=   $this->Masdocfile->find('all',array('conditions'=>array('OfferNo'=>$OfferNo),'group'=>array('DocType')));
			
			$data		=   $this->Masdocfile->query("SELECT * FROM Esignature_Document_Master WHERE EmpCode='$EmpCode'");
			
			$EmpDetails =   $this->Masjclrentry->find('first',array('fields'=>array('documentDone','EsignatureValidateRemarks','EsignatureValidateStatus'),'conditions'=>array('EmpCode'=>$EmpCode))); 
            ...
            $this->set('data',$data);
			$this->set('EmpDetails',$EmpDetails['Masjclrentry']);
        }
    ...
    public function update_esignature_status(){
		
		$EsignatureValidateStatus	=	$this->request->data['EsignatureValidateStatus'];
		$EsignatureValidateRemarks	=	$this->request->data['EsignatureValidateRemarks'];
		$OfferNo					=	base64_decode($this->request->data['ON']);
		$EmpCode					=	base64_decode($this->request->data['EC']);
		
		$this->Masjclrentry->updateAll(array('EsignatureValidateStatus'=>"'".$EsignatureValidateStatus."'",'EsignatureValidateRemarks'=>"'".$EsignatureValidateRemarks."'"),array('EmpCode'=>$EmpCode));  
		
		$this->Session->setFlash('<span style="font-weight:bold;color:green;" >Your request update successfully.</span>'); 
        $this->redirect(array('action'=>'viewdetails','?'=>array('ON'=>base64_encode($OfferNo),'EC'=>base64_encode($EmpCode))));
   ...
   public function getcostcenter(){
	...
	public function updatestatus(){
		$id	=	$_REQUEST['id'];
		$this->Masdocfile->query("UPDATE `Esignature_Document_Master` SET EsignatureStatus='Received' WHERE Id='$id'");
		die;             
    }
