<?php
class SalaryslipPrintsController extends AppController {
    ...
    public function show_report(){
        ...
        if(isset($_REQUEST['BranchName']) && $_REQUEST['BranchName'] !=""){
            ...
            $data     =   $this->SalarData->find('all',array('conditions'=>$conditoin));
            
            if(!empty($data)){
                foreach($data as $row){
                    
                    $data = $this->Masjclrentry->find('first',array('fields'=>array('Dept','EmpType'),'conditions'=>array('EmpCode'=>$row['SalarData']['EmpCode'])));
                    
                    $EmpName            =   $row['SalarData']['EmpName'];
                    $EmpCode            =   $row['SalarData']['EmpCode'];
                    $Branch             =   $row['SalarData']['Branch'];
                    $Designation        =   $row['SalarData']['Designation'];
                    $Department         =   $data['Masjclrentry']['Dept'];
                    $EPFNo              =   $row['SalarData']['EPFNo'];
                    $ESICNo             =   $row['SalarData']['ESICNo'];
                    $WorkingDays        =   $row['SalarData']['WorkingDays'];
                    $EarnedDays         =   $row['SalarData']['EarnedDays'];
                    $Basic              =   $row['SalarData']['Basic1'];
                    $HRA                =   $row['SalarData']['HRA1'];
                    $Bonus              =   $row['SalarData']['Bonus1'];
                    $Conv               =   $row['SalarData']['Conv1'];
                    $PersonalAllowance  =   $row['SalarData']['Portfolio1'];
                    $MedicalAllowance   =   $row['SalarData']['MedicalAllowance1'];
                    $SpecialAllowance   =   $row['SalarData']['SpecialAllowance1'];
                    $OtherAllowance     =   $row['SalarData']['OtherAllowance1'];
                    $Arrear             =   $row['SalarData']['Arrear'];
                    $Incentive          =   ($row['SalarData']['Incentive']+$row['SalarData']['ExtraDayIncentive']+$row['SalarData']['PLI']);
                    $TotalEarnData      =   ($Basic+$HRA+$Bonus+$Conv+$PersonalAllowance+$MedicalAllowance+$SpecialAllowance+$OtherAllowance+$Arrear+$Incentive);
                    $TotalEarn          =   addslashes($this->moneyFormatIndia($TotalEarnData));
                    $EPF                =   $row['SalarData']['EPF'];
                    $ESIC               =   $row['SalarData']['ESIC'];
                    $LoanDed            =   $row['SalarData']['LoanDed'];
                    $AdvPaid            =   $row['SalarData']['AdvPaid'];
                    $OtherDeduction     =   $row['SalarData']['TotalDeduction'];
                    $TotalDedData       =   ($EPF+$ESIC+$LoanDed+$AdvPaid+$OtherDeduction);
                    $TotalDed           =   addslashes($this->moneyFormatIndia($TotalDedData));
                    $GrossSalary        =   $row['SalarData']['TaxTotalGross'];
                    $Exemption          =   "";
                    $Balance            =   "";
                    $Deduction          =   "";
                    $GrossTotal         =   "";
                    ...
                    $TaxOnTotal         =   "";
                    $TaxPayble          =   "";
                    $IncomeTax          =   $row['SalarData']['IncomeTax'];
                    $ChequeNo           =   $row['SalarData']['ChequeNumber'];
                    $NetSalary          =   addslashes($this->moneyFormatIndia($row['SalarData']['NetSalary']));
                    $NetSalaryInWord    =   addslashes($this->convertNumberToWordsForIndia($row['SalarData']['NetSalary']));
                    $SalDays            =   $row['SalarData']['SalDate'];
                    $EmpType            =   str_split($EmpCode, 3);
                    
                    if($EmpType[0] =="MAS"){
                        $logo           =   "maslogo.png";
                        $CompanyName    =   "Mas Callnet India Pvt. Ltd";
                    }
                    else if($EmpType[0] =="IDC"){
                        $logo           =   "idclogo.png";
                        $CompanyName    =   "ISPARK  Dataconnect  Pvt.  Ltd.";
                    }
      
                    if($EmpType[0] =="MAS" || $EmpType[0] =="IDC" && $data['Masjclrentry']['EmpType']=="ONROLL"){
                    
                        if($list_value!=''){									
                            $list_value=$list_value.",('".$EmpName."','".$EmpCode."','".$Branch."','".$Designation."','".$Department."','".$EPFNo."','".$ESICNo."','".$WorkingDays."','".$EarnedDays."','".$Basic."','".$HRA."','".$Bonus."','".$Conv."','".$PersonalAllowance."','".$MedicalAllowance."','".$SpecialAllowance."','".$OtherAllowance."','".$Arrear."','".$Incentive."','".$TotalEarn."','".$EPF."','".$ESIC."','".$LoanDed."','".$AdvPaid."','".$OtherDeduction."','".$TotalDed."','".$GrossSalary."','".$Exemption."','".$Balance."','".$Deduction."','".$GrossTotal."','".$AggOfChap."','".$TotalIncom."','".$TaxOnTotal."','".$TaxPayble."','".$IncomeTax."','".$ChequeNo."','".$NetSalary."','".$NetSalaryInWord."','".$SalDays."','".$logo."','".$CompanyName."')";
                        }
                        else{
                                         $list_value="('".$EmpName."','".$EmpCode."','".$Branch."','".$Designation."','".$Department."','".$EPFNo."','".$ESICNo."','".$WorkingDays."','".$EarnedDays."','".$Basic."','".$HRA."','".$Bonus."','".$Conv."','".$PersonalAllowance."','".$MedicalAllowance."','".$SpecialAllowance."','".$OtherAllowance."','".$Arrear."','".$Incentive."','".$TotalEarn."','".$EPF."','".$ESIC."','".$LoanDed."','".$AdvPaid."','".$OtherDeduction."','".$TotalDed."','".$GrossSalary."','".$Exemption."','".$Balance."','".$Deduction."','".$GrossTotal."','".$AggOfChap."','".$TotalIncom."','".$TaxOnTotal."','".$TaxPayble."','".$IncomeTax."','".$ChequeNo."','".$NetSalary."','".$NetSalaryInWord."','".$SalDays."','".$logo."','".$CompanyName."')";
                        } 
                    
                    }
                }

                $this->SalarData->query("TRUNCATE TABLE SalarySlipMaster");
                $this->SalarData->query("INSERT INTO SalarySlipMaster(
                `EmpName`,`EmpCode`,`Branch`,`Designation`,`Department`,`EPFNo`,`ESICNo`,
                `WorkingDays`,`EarnedDays`,`Basic`,`HRA`,`Bonus`,`Conv`,`PersonalAllowance`,
                `MedicalAllowance`,`SpecialAllowance`,`OtherAllowance`,`Arrear`,`Incentive`,
                `TotalEarn`,`EPF`,`ESIC`,`LoanDed`,`AdvPaid`,`OtherDeduction`,`TotalDed`,
                `GrossSalary`,`Exemption`,`Balance`,`Deduction`,`GrossTotal`,`AggOfChap`,
                `TotalIncom`,`TaxOnTotal`,`TaxPayble`,`IncomeTax`,`ChequeNo`,`NetSalary`,
                `NetSalaryInWord`,`SalDays`,`logo`,`CompanyName`) values $list_value");
                echo "1";die;
            }
            else{
                echo "";
                                ...
                                <div class="col-sm-12" style="border:1px solid #FFF;" >
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-12" style="text-align: right;">
                                            <button type="button" onclick="chequeVerification('verify')" id="back" class="btn btn-primary btn-new" style="width:50px;">Verify</button>
                                            <button type="button" onclick="printCheque('print')" class="btn btn-primary btn-new" style="width:50px;">Print</button>
                                            <button type="button" onclick="chequeVerification('delete')" id="back" class="btn btn-primary btn-new" style="width:50px;">Delete</button>
