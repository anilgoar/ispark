...
class ChequePrintsController extends AppController {
    ...
    public function beforeFilter(){
        parent::beforeFilter(); 
        $this->Auth->allow('index','previous_month_salary_print','show_report','printdetails','printdetailsc','getcostcenter','export_report','printcheque','insertchequedetails',
                           'chequeverified','chequeverification','chequenoexist','chequenoexistpremonth','chequecancel','chequecancel_previous_month');
    ...
    public function previous_month_salary_print(){
        $this->layout='home';
            ...
            <table border="1"  >     
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php  echo ($data['ChequeMaster']['CancelDate'] !=""?'=TEXT("'.$data['ChequeMaster']['CancelDate'].'","dd-mm-YYYY")':"");?></td>
                        <td><?php echo $data['ChequeMaster']['CancelBy']?></td>
                    </tr>
                    <?php }?>
                </tbody>
            </table>
        ...
        if($this->request->is('POST')){
            ...
            if(!empty($dataArr)){
                foreach($dataArr as $row){
                    ...
                    $this->ChequeMaster->query("UPDATE ChequePrintMaster SET CancelReason='$Reason',CancelDate=NOW(),CancelBy='$userid' WHERE Id='$Id'");   
                }
    ...
    public function chequecancel_previous_month(){
        $this->layout='home';
        ...
        if($this->request->is('POST')){
            ...
            if(!empty($dataArr)){
                ...
                $this->Session->setFlash('<span style="color:green;font-weight:bold;" >Your given cheque no cancel successfully.</span>'); 
                $this->redirect(array('action'=>'chequecancel_previous_month')); 
            }
            else{
                $this->Session->setFlash('<span style="color:red;font-weight:bold;" >This cheque no does not exist for this bank !</span>'); 
                $this->redirect(array('action'=>'chequecancel_previous_month')); 
                        ...
                        <div class="col-sm-7">
                            <div class="form-group" style="border:2px solid #436e90;padding:5px;background-color: #c0d6e4;line-height:25px;">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    -->
                                    <input type="text" name="PrintChequeDate" id="PrintChequeDate" class="datepik"  style="width:100px;" >
                                </div>
    ...
    public function chequenoexistpremonth(){ 
        $PrintCheckFrom =   $_REQUEST['PrintCheckFrom'];
        $PrintCheckTo   =   $_REQUEST['PrintCheckTo'];
        $PrintBankName  =   $_REQUEST['PrintBankName'];
        echo $this->ChequeMaster->find('count',array('conditions'=>"ChequeNumber between '$PrintCheckFrom' and '$PrintCheckTo' and BankName='$PrintBankName' AND CancelReason IS NULL"));die;
    }
