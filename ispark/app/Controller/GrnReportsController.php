...
class GrnReportsController  extends AppController 
{
    ...
    public $uses = array('Addbranch','Addcompany','CostCenterMaster','Tbl_bgt_expenseheadingmaster','Tbl_bgt_expensesubheadingmaster','Tbl_bgt_expenseunitmaster',
        ...
        'SalaryUpload','SalaryHead','SalaryUploadMaster','SalaryProfileType','Provision','InitialInvoice','PnlMaster','PnlBranchSave','PnlProcessSave','Logx');
...
$this->Auth->allow('index','grn_reject_report','export_grn_reject_report','pnl_revenue_report','export_pnl_revenue_report',
        ...
        'get_pnl_report_branch_wise','grn_gst_report','export_grn_gst_report','get_pnl_report_month_wise_summary','pnl_summary_report','UserLog');
...
//                    }
                    ...
                    $BMCcost1 = array('1'=>'BO/AHMH','2'=>'BO/DEL','3'=>'BSS/BO/CORP/107','4'=>'BO/HYD','5'=>'BO/JPR','6'=>'BO/KNL','7'=>'BO/MRT','8'=>'BO/CHD','9'=>'BO/','13'=>'BSS/BO/QUAL/257','14'=>'BSS/BO/AHMH-JD/430','15'=>'BSS/BO/NOIDA-DD/356','16'=>'BSS/BLD/NOI/ISPARK/353');
...
GROUP BY cm.branch   ORDER BY cm.branch"); */
       ...
       $ExpenseReport= $this->ExpenseMaster->query("SELECT IF(em.Vendor_State_Code=em.Billing_State_Code,'state','central')GSTType,em.gst_enable,vm.TDSEnabled,vm.TDS,vm.TDSSection,
vm.TallyHead,SUM(eep.Amount)Amount,eep.Rate,SUM(eep.Tax) Tax,SUM(eep.Total) Total,cm.Branch,vm.TDSTallyHead,
cm.Branch,em.FinanceYear,em.FinanceMonth,tsl.state_list,tsl.state_tally_code,tsl.state_list,PanNo FROM `expense_entry_master` 
em INNER JOIN expense_entry_particular eep ON em.Id=eep.ExpenseEntry  
INNER JOIN cost_master cm ON cm.Id = eep.CostCenterId
INNER JOIN branch_master bm ON cm.branch = bm.branch_name
INNER JOIN tbl_vendormaster vm ON vm.Id = em.vendor
INNER JOIN `tbl_state_list` tsl ON tsl.state_list = em.Vendor_State_Code

INNER JOIN `tbl_bgt_expenseheadingmaster` head ON em.HeadId = head.HeadingId
INNER JOIN `tbl_bgt_expensesubheadingmaster` subhead ON em.SubHeadId = subhead.SubHeadingId
WHERE 1=1 $qry  em.FinanceYear='$year' AND em.FinanceMonth='Feb'
GROUP BY cm.branch,em.id  ORDER BY cm.branch"); 
...
public function pnl_branch_wise_report()
  {
        ...
        $LastLog =   $this->UserLog("Finance Report/PnL/P&L Branch Wise Report");
 ...
 public function get_pnl_report_branch_wise() 
 {
    ...
    $this->set('month_report',$month);
      ...
      $qry3 = " ";
      $qry4 = " ";
      $this->set('branch_master1',$this->Addbranch->find('list',array('fields'=>array('id','branch_name'),'conditions'=>"active='1' and BranchPnl='A' $qu1")));
      $this->set('branch_master2',$this->Addbranch->find('list',array('fields'=>array('id','branch_name'),'conditions'=>"active='1' and BranchPnl='B' $qu1")));
      $this->set('branch_master3',$this->Addbranch->find('list',array('fields'=>array('id','branch_name'),'conditions'=>"active='1' and BranchPnl='C' $qu1")));
...
INNER JOIN cost_master cm ON pm.Cost_Center_OutSource = cm.cost_center 
 WHERE  pm.FinanceYear='{$Expense['FinanceYear']}' AND pm.`FinanceMonth1` = '{$Expense['FinanceMonth']}' $qu4 GROUP BY cm.OPBranch;"; 
      ...
      {
        $billing[strtoupper($pro['cm']['OPBranch'])] += $pro['0']['billing_amt'];
      }
...
INNER JOIN cost_master cm ON pm.Cost_Center_OutSource = cm.cost_center 
 WHERE Processed='1' and pm.FinanceYear='{$Expense['FinanceYear']}' AND pm.`FinanceMonth1` = '{$Expense['FinanceMonth']}' $qu4 GROUP BY cm.OPBranch;"; 
      ...
      {
        $billing_proc[strtoupper($pro['cm']['OPBranch'])] += $pro['0']['billing_amt'];
      }
...
//      }
 ...
 WHERE ti.invoiceType='Revenue' and ti.finance_year='{$Expense['FinanceYear']}' AND left(ti.`month`,3)='{$Expense['FinanceMonth']}' AND ti.`status`=0 $qu4 GROUP BY cm.OPBranch");
      foreach($InvoiceArr as $pro)
      {
          ...
          if($pro['cm']['Billing']=='1')
        {
            $billing_master_proc[$pro['cm']['OPBranch']] = $pro['0']['total'];
        }
      }
...
INNER JOIN cost_master cm ON pm.cost_center = cm.cost_center 
 WHERE  pm.FinanceYear='{$Expense['FinanceYear']}' AND pm.`FinanceMonth1` = '{$Expense['FinanceMonth']}' $qu4 GROUP BY cm.id;";  
      ...
      {
          $InDirect['Process Outsourcing']['CONTRACT FEES'][$pro['cm']['OPBranch']] += $pro['0']['billing_amt'];
          $UnInDirect['Process Outsourcing']['CONTRACT FEES'][$pro['cm']['OPBranch']] += $pro['0']['billing_amt'];
        //$billing_proc[$pro['cm']['id']] = $pro['0']['billing_amt'];
        $SubHeadInDir[] = 'Process Outsourcing';
        $HeadInDir[] = 'CONTRACT FEES';
        $BranchInNArr[] = strtoupper($pro['cm']['OPBranch']);
      }
       ...
       $PnlMaster = $this->PnlMaster->find('all',array('conditions'=>array('ForPnlType'=>array('Branch','Process','MPR'),'ShowPnl'=>array('2','3'))));
       ...
       foreach($PnlMaster as $pnl)
       {
           ...
           {
                $PnlProcessSave = $this->PnlProcessSave->find('all',array('conditions'=>"FinanceYear='{$Expense['FinanceYear']}' and FinanceMonth='{$Expense['FinanceMonth']}' and pnlMasterId='{$pnl['PnlMaster']['PnlMasterId']}' $qu5"));
        ...
        foreach($PnlProcessSave as $processRecords)
        {
            $Futur_Revenue[$processRecords['PnlProcessSave']['OPBranch']] += round($processRecords['PnlProcessSave']['PnlAmount']);
            //$Futur_Revenue[$processRecords['PnlProcessSave']['OPBranch']]['unproc'] += round($processRecords['PnlProcessSave']['PnlAmount']);
        }
        ...
        foreach($PnlProcessSave as $processRecords)
        {
            $MPR_Seat[$processRecords['PnlProcessSave']['OPBranch']]['proc'] += round($processRecords['PnlProcessSave']['Seat']);
            $MPR_Rate[$processRecords['PnlProcessSave']['OPBranch']]['unproc'] += round($processRecords['PnlProcessSave']['Rate']);
        }
       ...
       $FinanceArr = $this->ExpenseEntryMaster->query("SELECT subhead.SubHeadingDesc,head.HeadingDesc,cm.OPBranch,SUM(eep.Amount) Amount FROM expense_entry_master eem 
INNER JOIN expense_entry_particular eep ON eem.Id = eep.ExpenseEntry
INNER JOIN `tbl_bgt_expenseheadingmaster` head ON eem.HeadId = head.HeadingId
INNER JOIN `tbl_bgt_expensesubheadingmaster` subhead ON eem.SubHeadId = subhead.SubHeadingId
INNER JOIN cost_master cm ON eep.CostCenterId = cm.id
WHERE eem.FinanceYear='{$Expense['FinanceYear']}' and eem.FinanceMonth='{$Expense['FinanceMonth']}' AND eem.HeadId='23' $qu4
GROUP BY eep.BranchId,subhead.SubHeadingDesc"); 
       
       foreach($FinanceArr as $Dir)
       {
           $Finance[strtoupper($Dir['cm']['OPBranch'])] = $Dir['0']['Amount'];
       }
      
       
       $UnFinanceArr = $this->ExpenseMaster->query("SELECT eem.EntryStatus,subhead.SubHeadingDesc,head.HeadingDesc,cm.OPBranch,SUM(eep.Amount) Amount FROM expense_master eem 
INNER JOIN expense_particular eep ON eem.Id = eep.ExpenseId AND eep.ExpenseType='CostCenter'
INNER JOIN `tbl_bgt_expenseheadingmaster` head ON eem.HeadId = head.HeadingId
INNER JOIN `tbl_bgt_expensesubheadingmaster` subhead ON eem.SubHeadId = subhead.SubHeadingId
INNER JOIN cost_master cm ON eep.ExpenseTypeId = cm.id
...
GROUP BY eep.BranchId,subhead.SubHeadingDesc"); 
       ...
       foreach($UnFinanceArr as $Dir)
       {
           if($Dir['eem']['EntryStatus']=='0')
           {
                $FinanceUN[strtoupper($Dir['cm']['OPBranch'])] = $Finance[strtoupper($Dir['cm']['OPBranch'])];  
           }
           else
           {
                $FinanceUN[strtoupper($Dir['cm']['OPBranch'])] = $Dir['0']['Amount'];
           }
       }
       ...
       $PnlProcessSave = $this->PnlProcessSave->find('all',array('conditions'=>"FinanceYear='{$Expense['FinanceYear']}' and FinanceMonth='{$Expense['FinanceMonth']}' and pnlMasterId='8' $qu5"));       
        foreach($PnlProcessSave as $processRecords)
        {
            $Finance[$processRecords['PnlProcessSave']['OPBranch']] += round($processRecords['PnlProcessSave']['PnlAmount']);
            $FinanceUN[$processRecords['PnlProcessSave']['OPBranch']] += round($processRecords['PnlProcessSave']['PnlProcAmount']);
       ...
       $out_qr = "SELECT * FROM tbl_invoice ti INNER JOIN cost_master cm ON ti.cost_center=cm.cost_center and cm.active='1'
...
date(LAST_DAY(SUBDATE(STR_TO_DATE(CONCAT(YEAR(CURDATE()),'-','{$Expense['FinanceMonth']}','-','01'),'%Y-%b-%d'),INTERVAL 1 MONTH)))";
        ...
        foreach($outstanding_proc_to_date as $out)
        {
            ...
            $branch_name_out_opt = $out['cm']['OPBranch'];
            ...
            $outstand_cost[$out['cm']['cost_center']] += $out['ti']['total'];
            $outstand_branch[$out['cm']['cost_center']] = $branch_name_out_opt;
        }
        
        
        //print_r($outstand_proc_data); exit;
       $rev_provision_qr = "SELECT * FROM provision_master pm INNER JOIN cost_master cm ON pm.cost_center=cm.cost_center  and cm.active='1'
...
LAST_DAY(SUBDATE(STR_TO_DATE(CONCAT(YEAR(CURDATE()),'-','{$Expense['FinanceMonth']}','-','01'),'%Y-%b-%d'),INTERVAL 1 MONTH))";  
       $outstanding_unproc_to_date = $this->InitialInvoice->query($rev_provision_qr);
        ...
        {
            $outstand_uncost[$out['cm']['cost_center']] += $out['pm']['provision'];
            $outstand_branch[$out['cm']['cost_center']] = $out['cm']['OPBranch'];
        }
        ...
        {
            $outstand_unproc_data[$outstand_branch[$cost]] += ($outstand_uncost[$cost] - $outstand_cost[$cost])*1.18;
            $outstand_unproc_cost[$cost] += ($outstand_uncost[$cost] - $outstand_cost[$cost]);
 ...
 public function grn_gst_report()
{
    ...
    foreach($outstanding_to_date as $out)
    {
        $FinanceYear_out = $out['ti']['finance_year'];
        $FinanceMonth_out = $out['ti']['month'];
...
public function pnl_summary_report()
  {
        ...
        $LastLog =   $this->UserLog("Finance Report/PnL/P&L Summary Report");
        $this->set('LastLog',$LastLog);
  }
