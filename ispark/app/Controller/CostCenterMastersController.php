...
class CostCenterMastersController extends AppController 
{
    ...
    public function beforeFilter()
    {
        ...
        {   $role=$this->Session->read("role");
            ...
            $this->Auth->allow('index','add','view','edit','update','view_tmp','tmp_edit_cost','tmp_update_cost','get_GST_Type');
            $this->Auth->allow('index');$this->Auth->allow('add','get_GST_Type');
            $this->Auth->allow('view');$this->Auth->allow('add_particulars');
            $this->Auth->allow('view','edit','edit_cost','update_cost','disable_cost','get_GST_Type');$this->Auth->allow('update_cost');
            $this->Auth->allow('tmp_view','tmp_edit_cost','tmp_update_cost');
            $this->Auth->allow('tmp_view_first','tmp_edit_cost_first','tmp_update_cost_first');
            $this->Auth->allow('tmp_view_second','tmp_edit_cost_second','tmp_update_cost_second');
            $this->Auth->allow('tmp_view','tmp_edit_cost','tmp_update_cost','delete_particulars','get_GST_Type');
            
            $this->Auth->allow('tmp_view_pending','tmp_edit_cost_pending','tmp_update_cost_pending','delete_particulars','get_GST_Type');
        }
    }
    ...
    public function add() 
    {
        if ($this->request->is('post')) 
        {
            ...
            if ($this->TmpCostCenterMaster->save($data))
            {
                ...
                $this->TmpCostParticular->updateAll(array('cost_master_id'=>$id),array('userid'=>$this->Session->read('userid'),'cost_master_id'=>null));
                    ...
                    WHERE bpp.bill_no IS NULL"))
                {
                    if(!$this->CostCenterMaster->query("INSERT INTO cost_master_disable SELECT * FROM cost_master WHERE id='$id'"))
                    {
                        ...
                        if(!$this->CostCenterMaster->query("INSERT INTO cost_master_history(
                            cost_id,dialdesk_client_id,company_name,branch,stream,`process`,process_name,tower,category,`type`,`client`,total_man_date,shrinkage,attrition,shift,
                            ...
                            SELECT id,dialdesk_client_id,company_name,branch,stream,`process`,process_name,tower,category,`type`,`client`,total_man_date,shrinkage,attrition,shift,
                            working_days,target_mandate,over_saldays,training_days,incentive_allowed,training_attrition,deduction_allowed,description,process_manager,
                            emailid,contact_no,po_required,jcc_no,grn,bill_to,as_client,b_Address1,b_Address2,b_Address3,b_Address4,b_Address5,ship_to,as_bill_to,
                            a_address1,a_address2,a_address3,a_address4,a_address5,revenueType,`fixed`,variableBase,agreementReq,paymentMode,paymentTerms,AssociationDate,
                            goLiveDate,UserName1,UserName2,UserName3,UserDesignation1,UserDesignation2,UserDesignation3,UserContactNo1,UserContactNo2,UserContactNo3,
    ...
    public function update_cost()
    {
        if ($this->request->is('post')) 
        {
            ...
            if($this->CostCenterMaster->updateAll($dataX,array('id'=>$id)))
            {
                        ...
                        $url['req_id'] = '2';
                        $url['id'] = $id;
                        $url['branch_name'] = $data['branch'];
                        $url['process_name'] = $data['process_name'];
                        $url['process_active'] = $data['active'];
                       ...
                       # $urlName = "http://14.97.29.227/qa/api_for_bill/api_for_bill.php?".$postdata; 
                      #  $ch = curl_init();
...
cost_center_email cce ON cm.Id = cce.cost_center WHERE cm.cost_center = '$cost_center'");
            ...
            {
                    $this->Session->setFlash(__("<h4 class=bg-success>".'The cost Master could not be updated. Please Try Again!'."</h4>"));
                    return $this->redirect(array('action'=>'view'));					
            }
            $this->set('data',$dataX);
        }
    ...
    public function tmp_view_first()
    {
        $role=$this->Session->read("role");
        $roles=explode(',',$this->Session->read("page_access"));
    ...
    public function tmp_edit_cost_first()
    {			
            $id = $this->request->query['id'];
            $role=$this->Session->read("role");
            $roles=explode(',',$this->Session->read("page_access"));
    ...
    public function tmp_update_cost_first()
    {
        if ($this->request->is('post')) 
        {
            $submit = $this->request->data['submit'];
        ...
        $this->Session->setFlash(__($flash));
        return $this->redirect(array('action'=>'tmp_view_first'));
        }
    }
    
    public function tmp_view_second()
    {
        $role=$this->Session->read("role");
        $roles=explode(',',$this->Session->read("page_access"));
    ...
    public function tmp_edit_cost_second()
    {			
            $id = $this->request->query['id'];
            $role=$this->Session->read("role");
            $roles=explode(',',$this->Session->read("page_access"));
    ...
    public function tmp_update_cost_second()
    {
        if ($this->request->is('post')) 
        {
            $submit = $this->request->data['submit'];
            ...
            if($submit=='Approve')
            {
                    ...
                    $this->Addbranch->query("INSERT INTO `tbl_voucher_entries` SET entry_type='cost_center',part_id='$cost_center',part_name='$cost_center',createdate=NOW()");
        ...
        $this->Session->setFlash(__($flash));
        return $this->redirect(array('action'=>'tmp_view_second'));
        }
    }
