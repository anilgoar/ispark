...
class HomeTestsController extends AppController {

    public $uses=array('PageMaster','User','Access','Pages','Addbranch','InitialInvoice','CostCenterMaster','Provision','Logx','HisAction','NotificationMaster','EcrMaster');
        ...
        if (!empty($rdata))		//checking if array is not empty
	{
            ...
            'conditions'=> array( 'not' => array('bill_no' =>''),'id >' => '179','finance_year'=>'2016-17'),
            'order' => array("CONVERT(SUBSTRING_INDEX(bill_no,'/',1),UNSIGNED INTEGER)"=>'desc')
            ));
...
WHERE IF(cm.grn='Yes' AND (tb.grn IS NULL OR tb.grn ='') AND (tb.grn_date IS NULL OR tb.grn_date = ''),TRUE,FALSE)) AND finance_year='2016-17' AND tb.branch_name ='$branch_name' order by tb.branch_name")); 
           ...
           $this->set('provision3',$this->Provision->find('all',array('fields'=>array('branch_name','cost_center','finance_year','month','provision_balance'),
            'conditions'=>array('not'=>array('provision_balance'=>'0'),'branch_name' => $branch_name))));
            }
            else
            {
                $this->set('provision','');
