<?php
class AccountValidationsController extends AppController {
    ...
    public function index(){
        ...
        if($this->request->is('Post')){ 
            $branch_name    =   $this->request->data['AccountValidations']['branch_name'];
            ...
            $conditoin=array(
                ...
                'CancelledChequeImage !='=>NULL,
               	'OR'=> array('AcValidationStatus is null', 'AcValidationStatus'=>'No'), 
            );*/
      
            $conditoin=array('Status'=>1,'AcNo !='=>'','AccHolder !='=>'','AcValidationStatus'=>NULL);
            ...
            $data1   =   $this->Masjclrentry->find('all',array('conditions'=>$conditoin));
            ...
            foreach ($data1 as $val){
                $OfferNo=$val['Masjclrentry']['OfferNo'];
                $CanCheque=$this->Masdocfile->query("select filename from mas_docoments where `OfferNo`='$OfferNo' and DocType='PassBook' and `DocName`='Bank Details'");
                $Doc=$CanCheque[0]['mas_docoments']['filename'];
                //if($Doc !=""){
                    $data[]=$val;  
                //}  
            }
    ...
    public function viewdetails(){
        ...
        if(isset($_REQUEST['EJEID'])){
            ...
            $this->set('CancelCheque',$CanCheque[0]['mas_docoments']['filename']);
            $this->set('data',$data);
        }
        ...
        if($this->request->is('Post')){
            ...
            $updArr=array(
                'EmpName'=>"'".addslashes($this->request->data['AccHolder'])."'",
                'AccHolder'=>"'".addslashes($this->request->data['AccHolder'])."'",
                'AcBank'=>"'".addslashes($this->request->data['AcBank'])."'",
                'AcBranch'=>"'".addslashes($this->request->data['AcBranch'])."'",
                'AcNo'=>"'".addslashes($this->request->data['AcNo'])."'",
                'IFSCCode'=>"'".addslashes($this->request->data['IFSCCode'])."'",
                'AccType'=>"'".addslashes($this->request->data['AccType'])."'",
                'AcRejectionRemarks'=>"'".addslashes($this->request->data['AcRejectionRemarks'])."'",
                'AcValidationDate'=>"'".addslashes(date('Y-m-d H:i:s'))."'",
                'AcValidatedBy'=>"'".addslashes($AuthId)."'",
                'AcValidationStatus'=>"'".addslashes($Status)."'",
            );
            
            if($this->Masjclrentry->updateAll($updArr,array('id'=>$EJEID))){
                ...
                $this->Session->setFlash('<span style="color:red;font-weight:bold;" >Your request does not update please try again later.</span>');
                $this->redirect(array('action'=>'viewdetails','?'=>array('EJEID'=>base64_encode($EJEID))));
            }
