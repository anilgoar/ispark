<?php
class TrainingAllocationBatchsController extends AppController {
    public $uses = array('Addbranch','Masjclrentry','Masattandance','FieldAttendanceMaster','OnSiteAttendanceMaster','TrainingAllocationMaster','TrainingBatchMaster');
        
    public function beforeFilter(){
        parent::beforeFilter(); 
        $this->Auth->allow('index','getbatchcode','showtrainingemployee','showallocatetraining','allocateinbatch','removefrombatch');
    ...
    public function allocateinbatch(){
        $this->layout='ajax';
        if(isset($_REQUEST['BatchCode']) && $_REQUEST['BatchCode'] !=""){
            $BachCode   =   $_REQUEST['BatchCode'];
            $BioCode    =   $_REQUEST['BioCode'];
            $data       =   $this->TrainingBatchMaster->find('first',array('fields'=>array('BatchCount','Id'),'conditions'=>array('BatchCode'=>$BachCode)));
            $BatchCount =   $data['TrainingBatchMaster']['BatchCount'];
            $AllocCount =   $this->TrainingAllocationMaster->find('count',array('conditions'=>array('BatchCode'=>$BachCode)));
            $TotalCount =   $BatchCount-$AllocCount;
            $BatchId    =   $data['TrainingBatchMaster']['Id'];
            
            
            
           
            if($AllocCount >= $BatchCount){
                echo "Sorry in this batch avaliable $TotalCount sheet.";die;
            }
            else{
                foreach($BioCode as $val){
                    $dataArr    =   $this->Masattandance->query("SELECT * from mas_Jclrentrydata where AllocateInBatch is null and BioCode='$val'");
                    $result     =   $dataArr[0]['mas_Jclrentrydata'] ;
                    ...
                    $resultArr  =   array(
                        ...
                        'TrainingRequired'=>$result['TrainningStatus'],
                    );
                    
                    if($this->TrainingAllocationMaster->saveAll($resultArr)){
                        $ActualStartDate=date('Y-m-d');
                        $this->TrainingBatchMaster->query("UPDATE TrainingBatchMaster SET ActualStartDate='$ActualStartDate' where Id='$BatchId'");
                        $this->TrainingAllocationMaster->query("UPDATE mas_Jclrentrydata SET AllocateInBatch='Yes' WHERE BioCode='$val'");
                    }  
                } 
    ...
    public function removefrombatch(){
        ...
        if(isset($_REQUEST['BatchCode']) && $_REQUEST['BatchCode'] !=""){
            ...
            foreach($BioCode as $val){
                $this->TrainingAllocationMaster->query("delete from TrainingAllocationMaster WHERE BioCode='$val' and BatchCode='$BachCode'");
                $this->TrainingAllocationMaster->query("UPDATE mas_Jclrentrydata SET AllocateInBatch=NULL WHERE BioCode='$val'");
            }
            
            $AllocationCount        =   $this->TrainingAllocationMaster->find('count',array('conditions'=>array('BatchCode'=>$_REQUEST['BatchCode'])));
            
            if($AllocationCount == 0){
                $this->TrainingBatchMaster->query("UPDATE TrainingBatchMaster SET ActualStartDate=NULL,ActualEndDate=NULL where BatchCode='$BachCode'");
            }
            echo "";die;  
        }
    }
    ...
    public function showtrainingemployee(){
        $this->layout='ajax';
        if(isset($_REQUEST['BranchName']) && $_REQUEST['BranchName'] !=""){
            if($_REQUEST['BranchName'] !="ALL"){$condition="and BranchName= '{$_REQUEST['BranchName']}'";}else{$condition="";}
           
            $data = $this->Masattandance->query("SELECT * from mas_Jclrentrydata where AllocateInBatch is null and TrainningStatus='Yes' $condition");

            //$data = $this->Masattandance->query("SELECT * from Attandence a1 inner join mas_Jclrentrydata mj on a1.BioCode=mj.BioCode where mj.AllocateInBatch is null and a1.EmpCode is null and a1.PendingStatus = 1  $condition  group by a1.BioCode");  
            if(!empty($data)){  
            ?>
            <table class = "table table-striped table-hover  responstable">     
                <thead>
                    <tr>
                        ...
                        <th style="text-align:center;width:40px;">TrainningRequired</th>
                    </tr>
                </thead>
                <tbody>         
                <?php $i=1; foreach ($data as $post){ ?>
                <tr>
                    <td style="text-align:center;"><input type="checkbox" id="AllocateBioCode" name="AllocateBioCode[]" value="<?php echo $post['mas_Jclrentrydata']['BioCode'];?>" ></td>
            ...
            <?php   
                ...
                <table class = "table table-striped table-hover  responstable">     
                    ...
                    <tbody>
                        <tr><td colspan="5" style="text-align:left;" >Record Not Found</td></tr>
                    </tbody>
                </table>
                <?php
            }
    ...
    public function showallocatetraining(){
        $this->layout='ajax';
        if(isset($_REQUEST['BranchName']) && $_REQUEST['BranchName'] !=""){
            ...
            $data = $this->Masattandance->query("SELECT * from TrainingAllocationMaster where BatchCode='{$_REQUEST['BatchCode']}' $condition");
            ...
            <table class = "table table-striped table-hover  responstable">     
                ...
                <?php $i=1; foreach ($data as $post){ ?>
                <tr>
                    <td style="text-align:center;"><input type="checkbox" id="RemoveBioCode" name="RemoveBioCode[]" value="<?php echo $post['TrainingAllocationMaster']['BioCode'];?>" ></td>
                    <td style="text-align:center;"><?php echo $i++; ?></td>
                    ...
                    <td style="text-align:center;"><?php echo $post['TrainingAllocationMaster']['TrainingRequired'];?></td>
                </tr>
                <?php } ?>            
                </tbody>   
            </table>
