...
function search_employees(EmpName){
    ...
    if(BranchName ===""){
        ...
        $.post("<?php echo $this->webroot;?>LeaveManagements/get_emp",{'EmpName':$.trim(EmpName),'BranchName':BranchName}, function(data) {
...
function validateEl(BranchName,EmpNameCode){
    var posts = $.ajax({type: 'POST',url:"<?php echo $this->webroot;?>LeaveManagements/validate_el",async: false,dataType: 'json',data: {BranchName:BranchName,EmpNameCode:EmpNameCode},done: function(response) {return response;}}).responseText;	
    return posts;
}


function existAttendance(BranchName,EmpNameCode,FromDate,ToDate){
	var posts = $.ajax({type: 'POST',url:"<?php echo $this->webroot;?>LeaveManagements/exist_attendance",async: false,dataType: 'json',data: {BranchName:BranchName,EmpNameCode:EmpNameCode,FromDate:FromDate,ToDate:ToDate},done: function(response) {return response;}}).responseText;	
	return posts;
}

function checkDate(FromDate,ToDate){
    var posts = $.ajax({type: 'POST',url:"<?php echo $this->webroot;?>LeaveManagements/check_date",async: false,dataType: 'json',data: {FromDate:FromDate,ToDate:ToDate},done: function(response) {return response;}}).responseText;	
    return posts;
}
...
function validateDateType(LeaveType,LeaveFor,FromDate,ToDate){
    ...
    var posts = $.ajax({type: 'POST',url:"<?php echo $this->webroot;?>LeaveManagements/validate_leave_with_date",async: false,dataType: 'json',data: {LeaveType:LeaveNo,LeaveFor:LeaveFor,FromDate:FromDate,ToDate:ToDate},done: function(response) {return response;}}).responseText;	
    return posts;
}
...
function getEmployeesDetails(EmpCode){
    ...
    $.post("<?php echo $this->webroot;?>LeaveManagements/get_gender",{'EmpCode':EmpCode,'BranchName':BranchName}, function(data) {
        $("#MtrlPtrl").html(data);
    });
    
    $.post("<?php echo $this->webroot;?>LeaveManagements/get_leave_details",{'EmpCode':EmpCode,'BranchName':BranchName}, function(data) {
        if(data !=""){
            $("#LeaveTable").show();
...
function getEmployeesDetails_by_month(){
    ...
    $.post("<?php echo $this->webroot;?>LeaveManagements/get_leave_details",{'EmpCode':EmpCode,'BranchName':BranchName,'FromDate':FromDate,'ToDate':ToDate}, function(data) {
        if(data !=""){
            $("#LeaveTable").show();
            $("#LeaveDetails").html(data);
        }
    ...
    <div class="col-xs-12 col-sm-12">
		...
		<div class="no-move"></div>
            ...
            <div class="box-content box-con">
                ...
                <?php echo $this->Form->create('LeaveManagements',array('class'=>'form-horizontal','action'=>'leaveentry','onSubmit'=>'return validateOdApply()')); ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <input type="text" name="FromDate" id="FromDate" onblur="getEmployeesDetails_by_month();" placeholder="Start Date"  autocomplete="off" class="form-control" required="" >
                    </div>
                    <div class="col-sm-2">
                        <input type="text" name="ToDate" id="ToDate" onblur="getEmployeesDetails_by_month();" placeholder="End Date" autocomplete="off" class="form-control" required="" >
                    </div>
                </div>
