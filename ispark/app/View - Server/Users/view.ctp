		...
		<div class="no-move"></div>
            ...
            <table id="table_id2" class="table table-striped table-bordered table-hover table-heading no-border-bottom">
                ...
                <?php 
                    ...
                    {
                        $det_provision3[$pr['Provision']['branch_name']][$i]['process'] = $pr['Provision']['cost_center'].'-'.$pr['cm']['process_name'];
                        $det_provision3[$pr['Provision']['branch_name']][$i]['finance_year'] = $pr['Provision']['finance_year'];
                        $det_provision3[$pr['Provision']['branch_name']][$i]['month'] = $pr['Provision']['month'];
                        $det_provision3[$pr['Provision']['branch_name']][$i++]['provision_balance'] = $pr['Provision']['provision_balance'];
                        $branchArr[] = $pr['Provision']['branch_name'];
                    }
                    $branchArr = array_unique($branchArr);
                    foreach($branchArr as $br)
                    {
                        $i = 1; $IniTotal = 0;
                        foreach($det_provision3[$br] as $pr):
                            echo "<tr>";
                            echo "<td>".$i++."</td>";
                            echo "<td>".$br."</td>";
                            echo "<td>".$pr['process']."</td>";
                            echo "<td>".$pr['finance_year']."</td>";
                            echo "<td>".$pr['month']."</td>";
                            echo "<td>".$pr['provision_balance']."</td>";
                            echo "</tr>";
                            $IniTotal +=$pr['provision_balance'];
                        endforeach;
