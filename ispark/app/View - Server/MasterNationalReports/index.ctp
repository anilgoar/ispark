...
<div class="row">
	...
	<div class="box">
            <div class="box-header">
                <div class="box-name"><i class="fa fa-table"></i><span>Master National Report</span></div>
                <div class="box-icons">
    ...
    <tbody>
        <?php $i=1; foreach ($master_report as $branch=>$mr): 
            ?>
            <tr class="">
                <td align="center"><?php echo $i; ?></td>
                <td align="center" class="MasterDayComp <?php echo $branch; ?>"><a href="#"><?php echo $branch; ?></a></td>
                ...
                <td align="center"><?php echo !empty($mr['PytReady'])?round($mr['PytReady']/100000,2):0; ?></td>
...
<div id="master_report_company_day"></div>
...
<script type="text/javascript">
    $(document).ready(function (){$('.MasterDayComp').click(function(){var postdata = this.className.split('MasterDayComp').join('').trim();
            var postdataArray=postdata.split("##");var branch = postdataArray[0];$.ajax({type:"Post",datatype:'json',cache:false,url: "MasterNationalReports/index2",
                data:{branch:branch}, success: function(data){$('#master_report_company_day').html(data);$('#master_report_day').html('');$('#master_report_day_brief').html('');
                }});});});$(document).on('click', '.MasterProcess', function(){var postdata = this.className.split('MasterProcess').join('').trim();var postdataArray=postdata.split("##");
           var branch = postdataArray[0];var company = postdataArray[1];$.ajax({type:"Post",datatype:'json',cache:false,url: "MasterNationalReports/get_master_process_wise",
           data:{branch:branch,company:company}, success: function(data){$('#master_report_day').html(data);$('#master_report_day_brief').html('');$('#master_report_event_brief').html('');}});});
    $(document).on('click', '.MasterProcessBrief', function(){var postdata = this.className.split('MasterProcessBrief').join('').trim();var postdataArray=postdata.split("##");
    var cost_center = postdataArray[0];var month = postdataArray[1];$.ajax({type:"Post",datatype:'json',cache:false,url: "MasterNationalReports/get_master_process_brief_wise",
        data:{cost_center:cost_center,month:month}, success: function(data){$('#master_report_day_brief').html(data);}});});$(document).on('click', '.MasterEventsBrief', 
function(){var postdata = this.className.split('MasterEventsBrief').join('').trim();var postdataArray=postdata.split("##");var id = postdataArray[0];
    $.ajax({type:"Post",datatype:'json',cache:false,url: "MasterNationalReports/get_master_event_brief_wise",data:{id:id}, success: function(data){$('#master_report_event_brief').html(data);}});});
