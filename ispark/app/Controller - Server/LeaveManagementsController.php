...
class LeaveManagementsController extends AppController {
    public $uses = array('Addbranch','Masjclrentry','Masattandance','LeaveManagementMaster','LeaveRightsMaster','FieldAttendanceMaster');
        
    public function beforeFilter(){
        parent::beforeFilter(); 
        $this->Auth->allow('index','leaveentry','leaveapproval','discardapprovedleave','leavedetails','get_emp','get_gender','get_leave_details','exist_attendance','check_date','validate_leave_with_date','validate_el');
        ...
        if(isset($_REQUEST['EmpCode']) && trim($_REQUEST['EmpCode']) !=""){
            ...
            if(!empty($data)){
                ...
                //if($months >=12){
                    ...
                    WHERE EmpCode='{$data['Masjclrentry']['EmpCode']}' AND YEAR(SalayDate) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR));");
    ...
    public function exist_attendance(){
        ...
        $EmpLocArr  =   $this->Masjclrentry->query("SELECT EmpLocation FROM masjclrentry WHERE EmpCode='$EmpCode' AND EmpType='ONROLL'");
