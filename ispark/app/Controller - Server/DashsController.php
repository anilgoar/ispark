...
class DashsController extends AppController 
{
	...
	else
	{
            ...
            $this->Auth->allow('dashboard','provisionDetails','showReport','get_dash_data','get_report11','view4','get_cost_center','view_process_report',
                    'view_process_report_freezed','get_actual_data','get_basic_direct_data','get_basic_indirect_data');
	}
    }
...
//                }
    ...
    WHERE pm.invoiceType1='Revenue' and pm.month='$NewFinanceMonth'");
...
WHERE   pm.FinanceMonth='$NewFinanceMonth' ");
...
`commit` Revenue,
direct_cost DirectCost,
indirect_cost InDirectCost,DATE_FORMAT(dd.createdate,'%d-%b-%y') LastUpdateDate
FROM `dashboard_data` dd
INNER JOIN cost_master cm ON dd.cost_centerId=cm.id $cost_id
WHERE YEAR(dd.createdate)='$YearRes'  AND dd.FinanceYear='$finYear' AND dd.FinanceMonth='$finMonth' AND  dd.branch='$Branch'  AND 
dd.createdate = (SELECT MAX(createdate) FROM dashboard_data AS dd1 WHERE YEAR(dd.createdate)='$YearRes'  
...
AND em.branch='$Branch' ");
    ...
    public function view_process_report_freezed()
    {
        ...
        $Select_Freeze_Data = "SELECT dfds.*,cm.cost_center,cm.process_name FROM `dashboard_freeze_data_save` dfds inner join cost_master cm on dfds.CostCenterId=cm.id WHERE dfds.Branch='$Branch' AND dfds.FinanceYear='$finYear' AND dfds.FinanceMonth='$finMonth'";
        $Freeze_Data = $this->FreezeData->query($Select_Freeze_Data);
