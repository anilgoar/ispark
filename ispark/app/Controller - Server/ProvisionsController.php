               ...
               $data=Hash::remove($data,'id');
                ...
                if($data['billing_bal']>=0)
                    {    
                        ...
                        if($this->Provision->updateAll(array('branch_name'=>$data['branch_name'],'cost_center'=>$data['cost_center'],'finance_year'=>$data['finance_year'],'month'=>$data['month'],'billing_amt'=>$data['billing_amt'],'billing_bal'=>$data['billing_bal']),array('id'=>$id)))
                        {
...
WHERE pm.billing_active='1' AND cm.Billing='1' AND pm.branch_name='$Branch' 
      AND pm.finance_year='$FinanceYear' AND pm.month='$monthNew' ORDER BY  RIGHT(cm.cost_center,3)"; 
...
INNER JOIN cost_master cm ON pm.cost_center = cm.cost_center AND cm.active='1'
WHERE pm.billing_active='1'   AND pm.branch_name='$Branch' and pm.cost_center='$CostCenter'
      AND pm.finance_year='$FinanceYear' AND pm.month='$monthNew' ORDER BY  RIGHT(cm.cost_center,3)";  
    ...
    public function get_cost_rev()
    {
        ...
        //$cost_list = $this->CostCenterMaster->find('list',array('fields'=>array('id',"cost_center"),"conditions"=>"branch='$Branch'",'order'=>array("RIGHT(CostCenterMaster.cost_center,3) asc")));
        $cost_list = $this->CostCenterMaster->query("SELECT cost_center,process_name FROM cost_master cm"
                . " WHERE branch='$Branch' and revenue='1' and active='1' ORDER BY  RIGHT(cost_center,3)");
...
WHERE pm.billing_active='1' AND cm.Billing='1' AND pm.branch_name='{$provision['branch_name']}' and pm.cost_center='{$provision['cost_center']}'
      AND pm.finance_year='{$provision['finance_year']}' AND pm.month='$monthNew' ORDER BY  RIGHT(cm.cost_center,3) limit 1"; 
       ...
       $this->set('branch_master',$branch);
        ...
        $table_arr = $this->ProvisionPart->query("SELECT * FROM `provision_particulars` ppt WHERE Branch='{$provision['branch_name']}' AND FinanceYear='{$provision['finance_year']}' AND FinanceMonth='{$provision['month']}' and Cost_Center='{$provision['cost_center']}'");
