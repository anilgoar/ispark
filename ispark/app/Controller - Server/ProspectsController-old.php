		...
		//$this->layout='home';
                ...
                else
                {
                    ...
                    if(in_array('90',$roles)){$this->Auth->allow('index','add','edit','save_sales','view_sales','create_cover','view_approve_sales',
                            'approve_sales','view_pdf','view_report','export_report','view_follow','follow_up','email_config','email_config_edit',
                            'lead_source_master','view_prospect_tracker','view_prospect_tracker_id','view_prospect_tracker_track','edit_sales');}
    ...
    public function save_sales() 
    {
        ...
        if($this->request->is('POST'))
        {
          ...
          { 
              ...
              $this->ProspectClient->updateAll(array('logo_file'=>"'$logo_name'",'attachment'=>"'$attachment'"),array('Id'=>$Id));
    ...
    public function create_cover() 
    {
        ...
        if($this->request->is("post"))
        {
            if($this->request->data['submit'])
            {
                ...
                $Lead['cc'] = addslashes($this->request->data['prospects']['EmailTo']);
            ...
            else if($this->request->data['Send'])
            {
                $Id = $this->request->data['prospects']['Id'];
                $this->ProspectClient->updateAll(array('IntroApprove'=>"'0'"),array('Id'=>$Id));
                ...
                {
                    //$ProductName = $this->ProspectProduct->find('first',array('conditions'=>array('Id'=>$ProductId)));
                    //$this->Session->setFlash(__("<h4 class=bg-success>".'Record has been saved successfuly'."</h4>"));
...
<body>
    ...
    <div style="margin-left:7px;" >
        ...
        <img src="http://www.mascallnetnorth.in'.$webroot.'img/pdfimg/P8.jpg" style=" width:713px;height:900px;" >
                    ...
                    $attachment[] =  $filename;
                    if(!empty($data['ProspectClient']['attachment']))
                        {
                            $attachment[] = '/var/www/html/ispark/app/webroot/prospect_file/'."$Id/".$data['ProspectClient']['attachment']; 
                        }
                    if(empty($to))
                    {
...
<body>
     <div class="pic1">
        <div style="margin-left:50%;margin-top:132px;">
            ...
            <p style="margin-left: 80px;"><img src="http://www.mascallnetnorth.in'.$webroot.'prospect_file/'."$v/".$ProductLogo.'" style="width:150px;" height="100px;" ></p>
    ...
    <div style="margin-left:7px;" >
        ...
        <img src="http://www.mascallnetnorth.in'.$webroot.'img/pdfimg/P8.jpg" style=" width:713px;height:900px;" >
                    ...
                    if(!empty($data['attachment']))
                        {
                            $attachment[] = '/var/www/html/ispark/app/webroot/prospect_file/'."$Id/".$data['attachment']; 
                        }
                        $mail = new sendEmailDialdesk();
                        $mail-> send_mail($to,$cc,$attachment,$body,$sub,$ProspectEmail['ProspectEmail']['Email_Host'],$ProspectEmail['ProspectEmail']['Email_Port'],$ProspectEmail['ProspectEmail']['Email_Id'],$ProspectEmail['ProspectEmail']['Email_Password']);
                    ...
                    $this->ProspectClient->updateAll(array('IntroApprove'=>"1",'Active'=>'3','ApproveBy'=>$this->Session->read("userid"),'ProspectUniqueNo'=>"'ProspectNo-".date('YmdHis')."'"),array("Id"=>$v));
        ...
        $this->set('sales_master', $this->ProspectClient->query("SELECT * FROM `prospect_client` sc INNER JOIN `prospect_product` sp ON sc.ProductId = sp.Id  INNER JOIN tbl_user tu ON sc.CreateBy=tu.Id WHERE  sc.Active='2' $qry AND
IF(Introduction ='Commercial' || Introduction ='Revised proposal',TRUE,FALSE) and IntroApprove=0 $childUser"));
    }
...
<body>
    ...
    public function view_report()
    {
        ...
        LEFT JOIN prospect_follow pf ON pc.Id = pf.ProspectId  LEFT JOIN tbl_user tu ON pc.CreateBy=tu.Id 
