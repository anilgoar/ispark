        ...
        else
        {
            ...
            $roles=explode(',',$this->Session->read("page_access"));
                $this->Auth->allow('tmp_view_branch_vendor','tmp_edit_branch_vendor','grn_payment','grn_payment_salary','vendor_add_head','add_head','add_sub_head');
                                        if(in_array('1',$roles)){$this->Auth->allow('index','imprest_save','imprest_manager_save','get_user','vendor_save','addunit','get_state_gst_code','view_vendor','vendor_save','edit_vendor','tmp_view_vendor','tmp_edit_vendor','add_tds_section','add_head_type','get_tds');}
            else if(in_array('77',$roles)){$this->Auth->allow('index','imprest_save','imprest_manager_save','get_user','vendor_save','addunit','get_state_gst_code','view_vendor','vendor_save','edit_vendor','tmp_view_vendor','tmp_edit_vendor');}
        }
    }
   ...
   public function vendor_save()
    {
        ...
        if($this->request->is('Post'))
        {
            ...
            if($this->VendorMaster->find('first',array('conditions'=>"Vendor='".$imprest['Vendor']."' and Id>620")))
            {
