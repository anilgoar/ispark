...
public function billApproval() 
{
    ...
    {		
        ...
        $result['bill_finance_year']=$finYear;
...
//                            $this->NotificationMaster->saveAll($nofifyArr);
		...
		public function reject_proforma()
		{
                   ...
                   $id  = base64_decode($this->request->query['id']); 
                    ...
                    if ($this->InitialInvoice->updateAll($data,array('id'=>$id)))
                    {
                       $prov_deduction = $this->ProvisionPartDed->query("Select * from provision_master_month_deductions pmmd where InvoiceId='$id' and deduction_status='1'");
                        foreach($prov_deduction as $pd)
                        {
                            $ProvisionId= $pd['pmmd']['ProvisionId'];
		...
		{	
                ...
                public function genrate_bill()
                {
                    ...
                    if ($this->request->is('post'))
                    {
                        ...
                        $bill = $this->BillMaster->query("SELECT MAX(BillNoChange) BillNoChange FROM tbl_invoice ti INNER JOIN cost_master cm ON ti.cost_center =cm.cost_center
                WHERE bill_finance_year='$f_year' AND ti.state_code = '$state_code' AND company_name ='$companyName' and ti.id!='$id'");
                        ...
                        $bill_no = $state_code.'-'.$idx.'/'.substr($f_year,2,6); 
