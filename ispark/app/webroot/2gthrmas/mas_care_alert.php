...
$qry ="SELECT * FROM `business_tickets` WHERE is_sender_find ='1'";
...
{    
    while($ticket_det=mysqli_fetch_assoc($ticket_list))
    {
        ...
        $mail_find_query = "select `to`,cc,bcc from business_rule where `type`='mascare' and costcenter='$costcenter' and department='$department' and branch = '$branch' limit 1"; 
        ...
        if(!empty($mail_find_det['to']))
        {
            ...
            $upd = "update business_tickets set is_sender_find='0',`to`='$to',cc='$cc',bcc='$bcc' where id='$ticket_id' limit 1";
            //echo $upd;
...
$qry_mail ="SELECT * FROM `business_tickets` WHERE email_send_status ='1' and is_sender_find='0'";
