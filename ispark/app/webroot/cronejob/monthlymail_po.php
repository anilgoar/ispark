...
WHERE SUBDATE(periodTo,INTERVAL 1 MONTH)=CURDATE()
...
$select = "SELECT id,branch_name,cost_center,periodTo FROM po_number WHERE SUBDATE(periodTo,INTERVAL 1 MONTH)=CURDATE() AND id in ($data_id)";
...
while($row=  mysql_fetch_assoc($excute))
{
    ...
    $emaildata['EmailText'] ="<table><tr><td style=\"padding-left:12px;\">The PO For Cost Center is $cost_center will be expired on $endDate. Please be sure New PO "
