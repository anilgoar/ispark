...
(SELECT id,data_id FROM po_number_particulars 
WHERE SUBDATE(periodTo,INTERVAL 15 DAY)=CURDATE()
...
$select = "SELECT id,branch_name,cost_center,periodTo FROM po_number WHERE SUBDATE(periodTo,INTERVAL 15 DAY)=CURDATE() AND id in ($data_id)";
...
while($row=  mysql_fetch_assoc($excute))
{
    ...
    $select3 = "SELECT * FROM `escalation2_table` WHERE data_id='$id' AND esc_id=1;";
    ...
    $emaildata['EmailText'] ="<table><tr><td style=\"padding-left:12px;\">The Agreement For Cost Center is $cost_center will be expired on $endDate. Please be sure New Agreement "
    ...
    if(!empty($mails['external_to']))
    {
        ...
        $emailexternal['EmailText'] ="<table><tr><td style=\"padding-left:12px;\">The PO For Cost Center is $cost_center will be expired on $endDate. Plese be sure New PO "
            . "will be submitted soon</td></tr></table>";
