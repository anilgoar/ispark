...
IF(
DATEDIFF(CURDATE(),DATE(ti.po_createdate))>3 && 
ROUND(DATEDIFF(CURDATE(),DATE(ti.po_createdate))%3,0)=1 && 
DAYNAME(ti.po_createdate) = 'monday',1,0) `count2` 
 ...
 CONCAT(cm.company_name,ti.branch_name,ti.finance_year,SUBSTRING_INDEX(ti.bill_no,'/',1))
   ...
   IF(cm.po_required='Yes',IF(ti.approve_po='Yes',TRUE,FALSE),TRUE) AND 
   IF(cm.grn='Yes',IF(ti.approve_grn='Yes',TRUE,FALSE),TRUE) AND 
   rm.Invoiceid IS NULL AND bpp.bill_no IS NULL AND IF(rm.ExpDatesPayment>CURDATE(),TRUE,FALSE)";
