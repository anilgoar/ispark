...
$select = "SELECT id,branch_name,cost_center,periodTo FROM po_number WHERE CURDATE()>periodTo AND DAYNAME(CURDATE())='monday' || DAYNAME(CURDATE())='thursday' AND id in ($data_id)";
...
while($row=  mysql_fetch_assoc($excute))
{
    ...
    $emaildata['EmailText'] ="<table><tr><td style=\"padding-left:12px;\">The PO For Cost Center is $cost_center expired on $endDate. Please be sure New PO "
            . "Sumitted</td></tr></table>";
