...
$qry ="select * from emp_onboard_trigger_services where trigger_type='bgv' and ticket_status='1' and type='joiner' and DATE(created_at)=DATE_SUB(CURDATE(),INTERVAL 10 DAY) and (last_mail_status IS NULL || last_mail_status='Fail')";
$DataArr = mysqli_query($con,$qry);

$qry1 ="select * from emp_onboard_trigger_services where trigger_type='bgv' and ticket_status='1' and type='joiner'";
$checkBgv = mysqli_query($con,$qry1);

if(!empty($checkBgv))
{
    while($data=mysqli_fetch_assoc($checkBgv))
    {
        $branch = $data['branch'];
        ...
        $bio_qry = "select BranchName,CostCenter,EmpCode,EmpName,Doj,AdharId from masjclrentry where BranchName='{$branch}' and CostCenter='{$costcenter}' and EmpCode='{$emp_code}' limit 1";
        $bio_data = mysqli_query($con,$bio_qry);
        $bio_arr = mysqli_fetch_assoc($bio_data);

        $aadhar_id = $bio_arr['AdharId'];

        $bgv_qry = "select * from bgv_api where aadhar_no='$aadhar_id' limit 1";
        $bgv_data = mysqli_query($con,$bgv_qry);
        $bgv_arr = mysqli_fetch_assoc($bgv_data);

        if(!empty($bgv_arr))
        {
            mysqli_query($con,"update emp_onboard_trigger_services SET bgv_color='{$bgv_arr['color']}',ticket_close_at=NOW(),ticket_status='0',close_remarks='Close by System' WHERE ticket_no='{$ticket_no}' and ticket_status='1'");
...
function send_email_mail($ticket_no,$EmpCode,$empname,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name)
{
    ...
    #$Subject        = "Mas Callnet : Bgv Initiating"; 

    if(!empty($variance))
    {
        $Subject   = "Escalation ".$attempt." Ticket BGV Delayed by ".$variance; 
    ...
    if(!empty($variance))
    {
        $EmailText     .= "<p>Escalation ".$attempt." Ticket BGV Delayed by ".$variance."</p>";
    }

    $EmailText     .= "<p>A New Ticket <b>".$ticket_no."</b> has been assigned for initiaing BGV. Employee Name - <b>".$empname."</b>,Employee Code - <b>".$EmpCode."</b>, Date of Joining (from Ispark) - <b>".date_format(date_create($doj),"d-M-Y")."</b>,Process Name - <b>".$costcenter_name."</b>.</p>";
    $EmailText     .= "<p>Please Create raise BGV request.</p>";
    ...
    if($done == '1')
    {
        ...
        $status .= 'Success';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='bgv',type='joiner',`to`='$to',cc='$cc',bcc='$bcc',mail_status='1',mail_resp='$status',mail_date=NOW()");

    }else{

        $status .= 'Fail';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='bgv',type='joiner',`to`='$to',cc='$cc',bcc='$bcc',mail_status='0',mail_body='{$done}',mail_resp='$status',mail_date=NOW()");
