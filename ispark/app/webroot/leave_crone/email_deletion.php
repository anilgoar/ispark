...
$qry ="select * from emp_onboard_trigger_services where trigger_type='email_id' and ticket_status='1' and type='leaver'";
$DataArr = mysqli_query($con,$qry);

$is_flag = false;


function send_email_mail($ticket_no,$EmpCode,$empname,$EmailId,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name)
{
    ...
    #$Subject        = "Mas Callnet : Email Deletion";
    ...
    $EmailText     .= "<p>A New Ticket <b>".$ticket_no."</b> has been assigned for deleting Email ID. Employee Name - <b>".$empname."</b>,Employee Code - <b>".$EmpCode."</b>, Date of Joining (from Ispark) - <b>".date_format(date_create($doj),"d-M-Y")."</b>,Email Id - <b>".$EmailId."</b>,Process Name - <b>".$costcenter_name."</b></p>";
...
if(!empty($DataArr))
{
    ...
    while($data=mysqli_fetch_assoc($DataArr))
    {
        ...
        $qry ="select * from emp_onboard_trigger_services where trigger_type='email_id' and emp_code = '{$emp_code}' and type = 'joiner'";
        $get_email = mysqli_query($con,$qry);
        $get_email_data = mysqli_fetch_assoc($get_email);
        if(!empty($get_email_data))
        {
            $empcode= $get_email_data['emp_code'];
            $mail_id = $get_email_data['close_remarks'];
    
        }else{
            
            $mail_id = $bio_arr['EmailId'];
        }
        ...
        send_email_mail($ticket_no,$EmpCode,$empname,$mail_id,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name);
