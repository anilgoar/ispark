...
$qry ="select * from emp_onboard_trigger_services where trigger_type='bio_id' and ticket_status='1' and type='leaver'";
$DataArr = mysqli_query($con,$qry);

$is_flag = false;

function send_bio_mail($ticket_no,$EmpCode,$empname,$biocode,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name)
{
    ...
    #$Subject        = "BioCode Deletion"; 

    if(!empty($variance))
    {
        $Subject   = "Escalation ".$attempt." Ticket Biometric Delayed by ".$variance; 
        
    }else{

        $Subject   = "Mas Callnet : BioCode Deletion"; 
    }
    ...
    if(!empty($variance))
    {
        $EmailText     .= "<p>Escalation ".$attempt." Ticket Biometric Delayed by ".$variance."</p>";
    }

    $EmailText     .= "<p style='font-weight: bold;'>A New Ticket <b>".$ticket_no."</b> has been assigned for Deleting Biometric ID. Employee Name - <b>".$empname."</b>,Employee Code - <b>".$EmpCode."</b>, Date of Joining (from Ispark) - <b>". date_format(date_create($doj),"d-M-Y")."</b>,Biometric Id - <b>".$biocode."</b>,Process Name - <b>".$costcenter_name."</b></p>";
    ...
    if($done == '1')
    {
        ...
        $status .= 'Success';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='bio_id',type='leaver',`to`='$to',cc='$cc',bcc='$bcc',mail_status='1',mail_resp='$status',mail_date=NOW()");

    }else{

        $status .= 'Fail';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='bio_id',type='leaver',`to`='$to',cc='$cc',bcc='$bcc',mail_status='0',mail_body='{$done}',mail_resp='$status',mail_date=NOW()");
...
if(!empty($DataArr))
{
    ...
    while($data=mysqli_fetch_assoc($DataArr))
    {
        ...
        $bio_qry = "select BranchName,CostCenter,EmpCode,EmpName,BioCode,Doj from masjclrentry where BranchName='{$branch}' and CostCenter='{$costcenter}' and EmpCode='{$emp_code}' limit 1";
        ...
        $biocode = $bio_arr['BioCode'];
        $doj = $bio_arr['Doj'];



        send_bio_mail($ticket_no,$EmpCode,$empname,$biocode,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name);
