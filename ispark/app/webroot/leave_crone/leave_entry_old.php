...
if(!empty($DataArr))
{
    ...
    while($data=mysqli_fetch_assoc($DataArr))
    {
        ...
        while($emp_arr = mysqli_fetch_assoc($emp_data))
        {
            ...
            if($a > 3 && empty($ins_qry[$emp_arr['EmpCode']]))
            {
    ...
    foreach($ins_qry as $key=>$data)
    {
        ...
        }else if(!empty($exist_from_data))
        {
            mysqli_query($con,"update continuously_leave SET to_date='{$data['end_date']}' WHERE EmpCode ='$key' and from_date='{$data['from_date']}'");
        }
        else
        {
