...
$qry ="select * from emp_onboard_trigger_services where trigger_type='bio_id' and ticket_status='1' and type='joiner'";
...
function send_bio_mail($ticket_no,$EmpCode,$empname,$biocode,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name)
{
    ...
    #$Subject        = "Mas Callnet : BioCode Creation"; 
    ...
    $EmailText     .= "<p>A New Ticket <b>".$ticket_no."</b> has been assigned for Biometric Creation. Employee Name - <b>".$empname."</b>,Employee Code - <b>".$EmpCode."</b>, Date of Joining (from Ispark) - <b>". date_format(date_create($doj),"d-M-Y")."</b>,Process Name - <b>".$costcenter_name."</b> .</p>";
    $EmailText     .= "<p>Please Create BIOMETRIC CODE.</p>";
    ...
    if($done == '1')
    {
        ...
        $status .= 'Success';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='bio_id',type='joiner',`to`='$to',cc='$cc',bcc='$bcc',mail_status='1',mail_resp='$status',mail_date=NOW()");

    }else{

        $status .= 'Fail';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='bio_id',type='joiner',`to`='$to',cc='$cc',bcc='$bcc',mail_status='0',mail_body='{$done}',mail_resp='$status',mail_date=NOW()");
