...
function check_file_exist($log_file_path,$emp_code,$search_string)
{
    ...
    foreach($file_list as $file_name)
    {
        ...
        $file_loc = "$log_file_path/log/$file_name";
         $emp_code.'<br/>'.$search_string."<br/>";
        $content = strtolower(file_get_contents($file_loc));
        #print_r($content);
        //echo gettype($content);exit; 
        
        if (strpos($content, strtolower($emp_code)) !== false && strpos($content, $search_string) !== false) {
                $return['status'] = true;
...
$qry ="select * from emp_onboard_trigger_services where trigger_type='ad_id' and ticket_status='1' and type='joiner'";
...
function send_email_mail($ticket_no,$EmpCode,$empname,$doj,$to,$cc,$bcc,$attempt,$con,$variance,$costcenter_name)
{
    ...
    if(!empty($variance))
    {
        ...
        $Subject   = "Mas Callnet : Ad Id Creation"; 
    }
    ...
    $EmailText     .= "<p>A New Ticket <b>".$ticket_no."</b> Ad ID creation. Employee Name - <b>".$empname."</b>,Employee Code - <b>".$EmpCode."</b>, Date of Joining (from Ispark) - <b>".date_format(date_create($doj),"d-M-Y")."</b>,Process Name - <b>".$costcenter_name."</b>.</p>";
    $EmailText     .= "<p>Please Create AD ID .</p>";
    ...
    if($done == '1')
    {
        ...
        $status .= 'Success';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='ad_id',type='joiner',`to`='$to',cc='$cc',bcc='$bcc',mail_status='1',mail_resp='$status',mail_date=NOW()");

    }else{

        $status .= 'Fail';
        mysqli_query($con,"insert emp_onboard_trigger_services_log SET ticket_no='{$ticket_no}',emp_code='$EmpCode',trigger_type='ad_id',type='joiner',`to`='$to',cc='$cc',bcc='$bcc',mail_status='0',mail_body='{$done}',mail_resp='$status',mail_date=NOW()");
