<script>
  ...
  {
      $.post("CostCenterActs/get_process",{branch:branch},function(data){
        $('#process').html(data);});
        document.getElementById("dash").innerHTML="";
  }
function costcenter(branch)
  {
      $.post("CostCenterActs/get_tower",{branch},function(data){
        $('#tower').html(data);});
  } 
    ...
    <div class="col-xs-12 col-sm-12">
        <div class="box">
            <div class="box-header">
                <div class="box-name"><span>Dashboard Close Process Window</span></div>
		<div class="box-icons">
            ...
            <div class="box-content"><h4 class="page-header"><?php echo $this->Session->flash(); ?></h4>
                ...
                echo $this->Form->create('Dashboard',array('class'=>'form-horizontal')); 
                    if(!empty($branchName))
                    {$countArr = array('branch'=>'Branch','cost_centerId'=>$tower1,'date'=>'Close Date');}
                    else
                    {
                        $countArr = array('cost_centerId'=>'cost_centerId','date'=>'Close Date');
                    }
                    ...
                    $flag = true;
                        for(; $i<$count; $i++)
                        {
                            ...
                            if($keys[$i]=='branch')
                            {
                                ...
                                echo $this->Form->input('branch',array('label'=>false,'options'=>$branchName,'empty'=>'Select Branch','onChange'=>'DashboardData(this.value)','required'=>true,'class'=>'form-control'));
                                echo '</div></div>';
                            }
