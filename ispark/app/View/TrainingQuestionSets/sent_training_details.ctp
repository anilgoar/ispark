...
function view_mail(){ 
    ...
    if(BranchName ===""){
        ...
        $.post("<?php echo $this->webroot;?>TrainingQuestionSets/send_view_mail",{BranchName:BranchName,Department:Department,Desgination:Desgination,Process:Process,Campaign:Campaign}, function(data){
            $("#view_mail").html(data);
        });      
    }
}
...
function sendemail(Id){  
    $(".loader").show();
    $.post("<?php echo $this->webroot;?>TrainingQuestionSets/sendusermail",{Id:Id}, function(data){
        $(".loader").hide();
        alert(data);
        view_mail();
    });  
...
function delete_mail(Id){
    if(confirm('Are you sure you want to delete this list item?')){
        window.location.href = "<?php echo $this->webroot;?>TrainingQuestionSets/sent_delete_mail?Id="+Id;
    }  
}
