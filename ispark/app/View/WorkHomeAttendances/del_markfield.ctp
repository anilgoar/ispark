...
function show_mark_list(value){
    var url="<?php $this->webroot;?>del_markfield?CSN=<?php echo base64_encode(isset($empArr['cost_center'])?$empArr['cost_center']:'');?>";
    if(value!=""){
        window.location.href = "<?php echo $this->webroot;?>WorkHomeAttendances/del_markfield?CSN=<?php echo base64_encode(isset($empArr['cost_center'])?$empArr['cost_center']:'');?>&mark_date="+value;
    }
    else{
        window.location.href = url;
    }
...
function goBack(){
    window.location="<?php echo $this->webroot;?>WorkHomeAttendances";  
}
...
function checkDate1(FromDate){
    ...
    var posts = $.ajax({type: 'POST',url:"<?php echo $this->webroot;?>WorkHomeAttendances/check_date",async: false,dataType: 'json',data: {FromDate:FromDate,CostCenter:CostCenter},done: function(response) {return response;}}).responseText;	
    return posts;  
}
    ...
    <div class="col-xs-12 col-sm-12">
        <div class="box">
            <div class="box-header"  >
                <div class="box-name">
                    <span>Discard Work Home Attendance</span>
		</div>
		<div class="box-icons">
		...
		<div class="no-move"></div>
            ...
            <div class="box-content box-con"> 
                <?php echo $this->Form->create('WorkHomeAttendances',array('class'=>'form-horizontal','action'=>'del_markfield')); ?>
                ...
                <?php echo $this->Form->create('WorkHomeAttendances',array('class'=>'form-horizontal','action'=>'del_attn','onsubmit'=>'return validatdFieldMark()')); ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2"> 
                            ...
                            <select  class="form-control"  name="MarkDate" id="MarkDate" onchange="show_mark_list(this.value)" required="">
                               <!-- <option value="">Select Date</option>-->
                               <?php foreach($dateArr as $dt){?>
                               <option  value="<?php echo $dt;?>" <?php if($mark_date==$dt) {echo 'selected';} ?>><?php echo date('d-M-Y',strtotime($dt));?></option>
                               <?php }?>
                           </select>
                        </div>
                        ...
                        <div class="col-sm-8">
                            <table class = "table table-striped table-hover  responstable" >         
                                ...
                                <tbody>         
                                    ...
                                    <tr>
                                        <td><?php echo $i++;?></td>
                                        <td><center><input class="checkbox" id="chkall_<?php echo $val['Masjclrentry']['Id'];?>"  type="checkbox" value="<?php echo $val['Masjclrentry']['EmpCode'];?>" name="check[]" checked="" ></center></td>
                    ...
                    <div class="form-group">
                        <div class="col-sm-8">
                            ...
                            <?php if($_REQUEST['STP'] !='search'){?>
                                <input  type='submit' class="btn btn-primary pull-right btn-new" value="Discard">
                            <?php }?>
                        </div>
                    </div>
