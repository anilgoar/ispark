...
function descardReport(Type){ 
    ...
    if(BranchName ===""){
        ...
        $("#loder").show();
        if(Type ==="show"){
            $.post("<?php echo $this->webroot;?>ProcessNocs/show_report",{BranchName:BranchName,StartDate:StartDate,CostCenter:CostCenter}, function(data) {
                $("#loder").hide();
                if(data !=""){
            ...
            $("#loder").hide();
            window.location="<?php echo $this->webroot;?>ProcessNocs/export_report?BranchName="+BranchName+"&StartDate="+StartDate+"&CostCenter="+CostCenter;  
           
        }
    }
}

function getLeftDetails(id){ 
    $("#msgerr").remove();
    $(".bordered").removeClass('bordered');
    
    $.post("<?php echo $this->webroot;?>ProcessNocs/viewdetails",{EJEID:id}, function(data) {
        $("#loder").hide();
        if(data !=""){
            $("#divLeft").html(data);
        }
        else{
            $("#divLeft").html('<div class="col-sm-12" style="color:red;font-weight:bold;" >Record not found.</div>');
        } 
    });
}
    ...
    <div class="col-xs-12 col-sm-12">
		...
		<div class="no-move"></div>
            ...
            <div class="box-content box-con">
                ...
                <?php echo $this->Form->create('ProcessNocs',array('action'=>'index','class'=>'form-horizontal','id'=>'showDetails')); ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <input onclick='return window.location="<?php echo $this->webroot;?>Menus/processnoc"' type="button" value="Back" class="btn btn-primary btn-new pull-right" style="margin-left: 5px;" />
                        <!--
                        <input type="button" onclick="descardReport('Export');" value="Export" class="btn pull-right btn-primary btn-new" style="margin-left:5px;" >
                        -->
