...
<table border="1" style="font-size: 90%;">
    ...
    <?php  $openOS = $freshBill = $realOB = $realFB = $total = $tds = $ded = $od = $odb = $netOS= $closeOS = $zero= $one = $two= $three = $four= $unPro = $totalOS =0;
            foreach($branch_master as $br){
                echo "<tr>";
                    echo "<td align='center'>".$br."</td>";
                    echo "<td align='center'>".$data_branch[$br]['openingOS']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['FreshBilling']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['RealisationOB']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['RealisationFB']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['Total']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['TDS']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['Deduction']."</td>";
                    echo "<td align='center'>".round($data_branch[$br]['other_deduction'])."</td>";
                    echo "<td align='center'>".round($data_branch[$br]['other_deduction_bill'])."</td>";
                    echo "<td align='center'>".round($data_branch[$br]['Total']-$data_branch[$br]['TDS']-$data_branch[$br]['Deduction']-$data_branch[$br]['other_deduction'])."</td>";
                    echo "<td align='center'>".round($data_branch[$br]['zero']+$data_branch[$br]['one']+$data_branch[$br]['two']+$data_branch[$br]['three']+$data_branch[$br]['four'])."</td>";
                    echo "<td align='center'>".$data_branch[$br]['zero']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['one']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['two']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['three']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['four']."</td>";
                    echo "<td align='center'>".$data_branch[$br]['Unprocess']."</td>";
                    echo "<td align='center'>".round($data_branch[$br]['zero']+$data_branch[$br]['one']+$data_branch[$br]['two']+$data_branch[$br]['three']+$data_branch[$br]['four']+$data_branch[$br]['Unprocess'])."</td>";
                    echo "<td align='center'>".$data_branch[$br]['remarks']."</td>";
                echo "</tr>";
                
                $openOS +=$data_branch[$br]['openingOS'];
                $freshBill += $data_branch[$br]['FreshBilling'];
                $realOB += $data_branch[$br]['RealisationOB'];
                 $realFB += $d['tab']['RealisationFB'];
                $total +=$data_branch[$br]['Total'];
                $tds +=$data_branch[$br]['TDS'];
                $ded += $data_branch[$br]['Deduction'];
                $od +=round($data_branch[$br]['other_deduction_bill']);
                $odb +=round($data_branch[$br]['other_deduction_bill']);
                $netOS +=round($data_branch[$br]['Total']-$data_branch[$br]['TDS']-$data_branch[$br]['Deduction']-$data_branch[$br]['other_deduction']);
                $closeOS +=round($data_branch[$br]['zero']+$data_branch[$br]['one']+$data_branch[$br]['two']+$data_branch[$br]['three']+$data_branch[$br]['four']);
                $zero +=$data_branch[$br]['zero'];
                $one +=$d['tab']['one'];
                $two +=$data_branch[$br]['two'];
                $three +=$data_branch[$br]['three'];
                $four +=$data_branch[$br]['four'];
                $unPro +=$data_branch[$br]['Unprocess'];
                $totalOS += round($data_branch[$br]['zero']+$data_branch[$br]['one']+$data_branch[$br]['two']+$data_branch[$br]['three']+$data_branch[$br]['four']+$data_branch[$br]['Unprocess']);
            }
    ?>
    <tr>
        <td align='center'><b>Total</b></td>
