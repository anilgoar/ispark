...
<div class="box-content" style="overflow:auto">
	...
	<?php echo $this->Form->create('Targets',array('class'=>'form-horizontal', 'url'=>'save_freeze_data','style'=>"overflow:auto;")); ?>			
...
<?php $flagSaveRequest = true;
    foreach($CostCenter as $cost_id=>$cost_master)
    {
        echo '<tr style="text-align:center">';
            ...
            echo '<td>';
                $Dir_com = round($Data[$cost_id]['Commit']['dc'],2);
            ...
            echo '<td>';
            $InDir_com = round($Data[$cost_id]['Commit']['idc'],2);
...
//            echo '</td>';
        ...
        if($Dir_bas!=$Dir_act && $flagSaveRequest)
        {
            $flagSaveRequest = false;
            $NotBifurcate = $cost_master['CostCenter'];;
        }
        else if($InDir_bas!=$InDir_act && $flagSaveRequest)
        {
            $flagSaveRequest = false;
            $NotBifurcate = $cost_master['CostCenter'];;
        }
...
?>
    ...
    <div class="form-group">
        <label class="col-sm-5 control-label"></label>
            <div class="col-sm-2">
                <?php if($flagSaveRequest) { ?>
                    <button type="submit" class="btn btn-primary btn-label-left">
                            Freeze Request 
                    </button>
                <?php  } else { ?>
                <button type="button" onclick="return confirm('Please Bifurcate New Basic Amount Direct and Indirect Cost - <?php echo $NotBifurcate; ?>');" class="btn btn-primary btn-label-left">
                            Freeze Request 
                    </button>
                <?php } ?>
            </div>
...
function get_total_mtd()
{
    ...
    for(var i=0; i<rate_arr.length; i++)
    {
        ...
        document.getElementById("mtd"+rate_arr[i]).value = mtdTotal.toFixed(2);
        document.getElementById("forcast"+rate_arr[i]).value = mtdTotal.toFixed(2);
    ...
    for(var i=0; i<other_arr.length; i++)
    {
        ...
        document.getElementById("mtd"+other_arr[i]).value = mtdTotal.toFixed(2);
        document.getElementById("forcast"+other_arr[i]).value = mtdTotal.toFixed(2);
    }
    document.getElementById("MtdTotal").value = amount;
    document.getElementById("ForecastTotal").value = amount;
}
