				...
				<table class="table table-striped table-bordered table-hover table-heading no-border-bottom">
						...
						<tr align="center">
                                ...
                                <td><?php if($data['IssueTracker']['issue_status']==0){echo "open";}
                                        ...
                                        else if($data['IssueTracker']['issue_status']==5){echo "Reject";}
                                        else if($data['IssueTracker']['issue_status']==6){echo "Allocated";}
								?>
							</td>
						</tr>
				 </table>
							...
							<td><?php echo $post['IssueParticular']['requirement_desc'];?></td>
                             ...
                             <td><?php 
                                ...
                                if($post['IssueParticular']['issue_status']=='3'){ 
                                    ...
                                    if($post['IssueParticular']['issue_status']=='5') echo "Allocated";
                                } ?>
                                </td>
                                <td><?php echo  $this->Form->input('remarks',array('label'=>false,'value'=>$post['IssueParticular']['remarks'])) ;?></td>
                                
                                <td> 
                                    <?php if($post['IssueParticular']['issue_status']=='3') { ?>
                                    <input type="submit" value="Submit"  action="update_issue_status" class="btn btn-info" /> 
                                    <?php } ?> 
                                </td>
                                
                                <td>
                                   <?php if($post['IssueParticular']['issue_status']=='1' || $post['IssueParticular']['issue_status']=='5') { echo $post['IssueParticular']['AllocateRemarks'];}?> 
                                </td>
                            </tr>
