			...
			<div class="box-content">
				...
				<table class="table table-striped table-bordered table-hover table-heading no-border-bottom">
						...
						<tr align="center">
                            ...
                            <!--
                            <td><?php if($post['UserIssue']['process_type']==1){echo "Inbound";}else{echo "Outbound";}  ?></td>
                            -->
                            <td><?php echo $post['UserIssue']['process_type']  ?></td>
                            <td><?php echo $post['UserIssue']['TicketCategory'];?></td>
			    ...
			    <td><?php  echo  isset($post['UserIssue']['start_date'])?date_format(date_create($post['UserIssue']['start_date']),'d-M-Y'):''; ?></td>
                            ...
                            <td>
			   <?php 
                           if(isset($user_check_status) && $user_check_status =="1"){
                            echo $this->Form->input('IssueTracker.'.$post['IssueTracker']['id'].'.issue_status',array('label'=>false,'options'=>array('Select Status','3'=>'Close','2'=>'In-progress','1'=>'On-hold'),'selected'=>$post['IssueTracker']['issue_status'],'class' => 'form-control')); 
                           }
                           else{
                              echo $this->Form->input('IssueTracker.'.$post['IssueTracker']['id'].'.issue_status',array('label'=>false,'options'=>array('Select Status','2'=>'In-progress','1'=>'On-hold'),'selected'=>$post['IssueTracker']['issue_status'],'class' => 'form-control'));  
                           }
                           ?>
                            </td>
