				...
				<?php $case=array('primary',''); $i=0; ?>
					<thead>
						...
						<tr class="<?php  echo $case[$i%4]; $i++;?>">
							<td><?php echo $i; ?></td>
							<td><code><?php echo $post['VendorMaster']['vendor']; ?></td>
							<td><code><?php echo $post['VendorMaster']['state']; ?></td>
							<td><code><?php echo $this->Html->link('Approve',array('controller'=>'Imprests','action'=>'tmp_edit_vendor','?'=>array('Id'=>$post['VendorMaster']['Id']),'full_base' => true)); ?></code></td>
						</tr>
						<?php endforeach; ?>
						<?php unset($Addclient); ?>
					</tbody>
