...
<?php //print_r($ExpenseEntryMaster); 
...
<div class="col-xs-12 col-sm-12">
    <div class="box">
        ...
        <div class="box-content" style="background-color:#ffffff; border:1px solid #436e90;">
            <h4 class="page-header textClass" style="border-bottom: 1px double #436e90;margin: 0 0 10px;">GRN Approval(Finance Head)<?php echo $this->Session->flash(); ?></h4>
            <!--
        ...
        'Jul'=>'Jul','Aug'=>'Aug','Sep'=>'Sep','Oct'=>'Oct','Nov'=>'Nov','Dec'=>'Dec'),'empty' => 'Select Month','value'=>$ExpenseEntryMaster[1],'label' => false,'id'=>'FinanceMonth', 'div' => false,'class'=>'form-control','selected' => $ExpenseMaster['1'],'readonly'=>$readonly)); ?>
            ...
            <div class="col-sm-4">
                <?php echo $this->Form->input('gst_enable',array('label' => false,'options'=>array('1'=>'Yes','0'=>'No'),'onChange'=>'getEnableStatus(this.value)','class'=>'form-control','empty'=>'Select','value'=>$ExpenseEntryMaster[17],'id'=>'gstEnable','required'=>true,'readonly'=>$readonly)); ?>
            </div>
            </div> 
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <?php echo $this->Form->input('Vendor_State_Code',array('label' => false,'options'=>$ExpenseEntryMaster[17]==1?$StateCode:array(),'class'=>'form-control','empty'=>'Select','value'=>$ExpenseEntryMaster[18],'id'=>'Vendor_State_Code','readonly'=>$readonly)); ?>
                </div>
                
                <label class="col-sm-2 control-label">Billing State Code</label>
                <div class="col-sm-4">
                    <?php echo $this->Form->input('Billing_State_Code',array('label' => false,'options'=>$ExpenseEntryMaster[17]==1?$StateCode:array(),'class'=>'form-control','empty'=>'Select','value'=>$ExpenseEntryMaster[19],'id'=>'Billing_State_Code','readonly'=>$readonly)); ?>
                </div>
            </div>
        ...
        <div class="form-group">						
            ...
            <div class="col-sm-2">
                ...
                <?php } ?>
                <a href="/ispark/Gms/view_approve_grn_finance_head" class="btn btn-info" >Back</a>
            </div>
        </div>    
        </div>
    </div>
    ...
    <div class="col-xs-12 col-sm-12">
		...
		<h4 class="page-header" style="border-bottom: 1px double #436e90;margin: 0 0 10px;">Details Entry</h4>
                    ...
                    <tr <?php  foreach ($result as $post): $GTotal1 += $post['teep']['Total']; endforeach; if($GTotal1==$ExpenseEntryMaster[7] && !empty($ExpenseEntryMaster)) { ?> style="display:none" <?php } ?> >
                        ...
                        <th><?php  if($ExpenseEntryMaster[17]=='1') $gst_en="1"; else { $gst_en="0";}  echo $this->Form->input('amount' ,array('label' =>false,'placeholder' => 'Amount','onBlur'=>"getTotalCostAmount('$gst_en','$ExpenseEntryMaster[18]','$ExpenseEntryMaster[19]')",'class' => 'form-control','onKeypress'=>'return isNumberKey(event)')); ?></th>
                        <th>
                        <?php if($ExpenseEntryMaster[17]=='1') { ?>
                        <?php echo $this->Form->input('Rate' ,array('label' =>false,'empty' => 'Select','options'=>array('5'=>'5%','12'=>'12%','18'=>'18%','28'=>'28%'),'class' => 'form-control','onChange'=>"getTotalCostAmount('$gst_en','$ExpenseEntryMaster[18]','$ExpenseEntryMaster[19]');",'onKeypress'=>'return isNumberKey(event)')); ?>
                        <?php } else {
                    ...
                    foreach ($result as $post): 
                        ...
                        $idx.=$post['teep']['Id'].','; ?>
                            ...
                            <td>
                                <?php 
                                if($ExpenseEntryMaster[17]=='1'  && $ExpenseEntryMaster[18]!=$ExpenseEntryMaster[19]){ 
                                    echo "CGST@".round($post['teep']['Rate']/2,2)."%"; echo "<br/>"; echo "SGST@".round($post['teep']['Rate']/2,2)."%"; 
                                } 
                                else if( $ExpenseEntryMaster[17]=='1'){ 
                                    echo "IGST@".round($post['teep']['Rate'])."%";
                                } 
                                else{  
                                    echo $post['teep']['Rate'];  
                                } 
                            ...
                            <td>
                                <?php 
                                if($ExpenseEntryMaster[17]=='1' && $ExpenseEntryMaster[18]!=$ExpenseEntryMaster[19]){ 
                                    echo round($post['teep']['Tax']/2,3); echo "<br/>"; echo round($post['teep']['Tax']/2,3); 
                                } 
                                else if($ExpenseEntryMaster[17]=='1'){
                                    echo round($post['teep']['Tax'],2);
                                } 
                                else{  
                                    echo $post['teep']['Rate'];
                                } 
                    ...
                    <div class="form-group">
                        ...
                        <?php if(strpos($ExpenseEntryMaster['12'], ',')) { $Files = explode(',',$ExpenseEntryMaster['12']); $files_no = 1;} else { $Files[] = $ExpenseEntryMaster['12']; } ?>
...
function getTotalCostAmount(con,VenStateCode,BillStateCode)
{
    ...
    else
    {
        ...
        if(con=='1')
        {
            ...
            if(VenStateCode ==BillStateCode){
                $('#Tax').html(tax);
            }
            else{
              $('#Tax').html((tax/2).toFixed(3)+"<br/>"+(tax/2).toFixed(3));   
            }
...
function update_grn() 
{
    ...
    var Billing_State_Code = $('#Billing_State_Code').val();
    ...
    {
        alert("Select GST Enable");
        $("#gst_enable").focus();
        return false;
    }
    else if(gst_enable=='1' && Vendor_State_Code=='')
    {
        alert("Select Vendor State Code");
        $("#Vendor_State_Code").focus();
        return false;
    }
    else if(gst_enable=='1' && Billing_State_Code=='')
    {
        alert("Select Billing State Code");
        $("#Vendor_State_Code").focus();
        return false;
    }
...
function getEnableStatus(Status){
    
    if(Status ==="1"){
        $.post("<?php echo $this->webroot;?>Gms/vendorstatuscode",function(data){
            $("#Vendor_State_Code").html(data);
            $("#Billing_State_Code").html(data);
        });
    }
    else{
        $("#Vendor_State_Code").html('');
        $("#Billing_State_Code").html('');
    }
}
</script>