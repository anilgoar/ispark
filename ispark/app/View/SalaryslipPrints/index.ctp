...
function salaryDetails(Type){ 
    ...
    if(BranchName ===""){
        ...
        $("#loder").show();
        if(Type ==="show"){
            $.post("<?php echo $this->webroot;?>SalaryslipPrints/show_report",{BranchName:BranchName,EmpMonth:EmpMonth,EmpYear:EmpYear,CostCenter:CostCenter,EmpLocation:EmpLocation,EmpCode:$.trim(EmpCode)}, function(data) {
                $("#loder").hide();
                if(data !=""){
                    window.open('<?php echo $this->webroot;?>app/webroot/checkprint/examples/salaryslip.php', 'Salary Slip', 'width=750, height=300, top=0, left=0');
                }
                else{
